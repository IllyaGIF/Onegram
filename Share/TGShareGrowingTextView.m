...
- (CGFloat)measureHeight
{
    if (iosMajorVersion() < 7)
        return floor(_internalTextView.contentSize.height);
    ...
    [[NSMutableAttributedString alloc] initWithAttributedString:_internalTextView.attributedText];
...
- (void)setText:(NSString *)newText animated:(bool)animated
{
    if (iosMajorVersion() < 6)
    {
        ...
        _placeholderView.hidden =
        _internalTextView.text.length != 0 || [_internalTextView isFirstResponder];
    ...
    [self setAttributedText:
     [[NSAttributedString alloc] initWithString:newText == nil ? @"" : newText
                                     attributes:[self defaultAttributes]]
                   animated:animated];
}
- (void)setAttributedText:(NSAttributedString *)newText animated:(bool)animated
{
    ...
    NSMutableAttributedString *fixedFontString =
    [[NSMutableAttributedString alloc] initWithAttributedString:newText];
    
    [fixedFontString removeAttribute:NSFontAttributeName
                               range:NSMakeRange(0, fixedFontString.length)];
    
    [fixedFontString addAttribute:NSFontAttributeName
                            value:_intrinsicTextFont == nil ? [UIFont systemFontOfSize:16.0f] : _intrinsicTextFont
                            range:NSMakeRange(0, fixedFontString.length)];
    ...
    _placeholderView.hidden =
    fixedFontString.length != 0 || [_internalTextView isFirstResponder];
...
- (void)refreshAttributes
{
    ...
    [_internalTextView.textStorage removeAttribute:NSForegroundColorAttributeName
                                             range:NSMakeRange(0, string.length)];
    
    if (_intrinsicTextColor != nil) {
        [_internalTextView.textStorage addAttribute:NSForegroundColorAttributeName
                                              value:_intrinsicTextColor == nil ? [UIColor blackColor] : _intrinsicTextColor
                                              range:NSMakeRange(0, string.length)];
    }
