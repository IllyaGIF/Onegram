...
- (void)setState:(TGCallSessionState *)state
{
    ...
    if (displayPeer != nil && _currentPeerId != displayPeer.uid)
    {
        _currentPeerId = displayPeer.uid;

        TGLog(@"CALL ui.avatar peer=%d photo=%d", displayPeer.uid, displayPeer.photoUrlSmall.length > 0 ? 1 : 0);
        ...
        {
            [_avatarView loadImage:displayPeer.photoFullUrlBig filter:@"circle:128x128" placeholder:placeholder];
        }
        else
        {
            [_avatarView loadUserPlaceholderWithSize:TGCallAvatarLargeSize uid:displayPeer.uid firstName:displayPeer.firstName lastName:displayPeer.lastName placeholder:placeholder];
        }
    }
}
