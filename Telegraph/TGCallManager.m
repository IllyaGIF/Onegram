...
- (SSignal *)requestCallWithPeerId:(int64_t)peerId {
    TGLog(@"CALL manager.request peer=%lld", peerId);
...
- (SSignal *)requestCallWithPeerId:(int64_t)peerId uuid:(NSUUID *)uuid session:(TGCallSession *)session {
    return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber) {
        ...
        [_queue dispatch:^{
            ...
            TGLog(@"CALL manager.create peer=%lld internal=%d uuid=%@", peerId, internalId, uuid);
            ...
            [self setContextState:internalId state:[[TGCallStateData alloc]
                                                    initWithInternalId:@(internalId)
                                                    callId:0
                                                    accessHash:0
                                                    state:TGCallStateRequesting
                                                    peerId:peerId
                                                    connection:nil
                                                    hungUpOutside:false
                                                    needsRating:false
            ...
            [context.disposable setDisposable:[[[TGCallSignals requestedOutgoingCallWithPeerId:peerId] deliverOn:_queue] startWithNext:^(id next) {
                TGLog(@"CALL manager.request.next peer=%lld internal=%d context=%@", peerId, internalId, next);
                ...
                TGLog(@"CALL manager.request.error peer=%lld internal=%d error=%@ rpc=%@", peerId, internalId, error, rpcError);
                ...
                if (strongSelf != nil) {
                    ...
                    TGCallDiscardedContext *discardedContext = [[TGCallDiscardedContext alloc]
                                                                initWithCallId:0
...
- (void)updateCallContextWithCallId:(int64_t)callId callContext:(id)callContext {
    [_queue dispatch:^{
        TGLog(@"CALL manager.update call=%lld incoming=%@ contexts=%d", callId, NSStringFromClass([callContext class]), (int)_callContexts.count);
        ...
        if (internalId == -1 && [callContext isKindOfClass:[TGCallAcceptedContext class]]) {
            ...
            [_callContexts enumerateKeysAndObjectsUsingBlock:^(NSNumber *key, TGCallContext *context, BOOL *stop) {
                if ([context.context isKindOfClass:[TGCallWaitingContext class]]) {
                    ...
                    if (waitingContext.callId == 0 &&
                        (waitingContext.participantId == acceptedContext.participantId ||
                         context.state.peerId == acceptedContext.participantId)) {
                            
                            TGLog(@"CALL manager.attachAccepted call=%lld peer=%d internal=%d",
                                  acceptedContext.callId,
                                  acceptedContext.participantId,
                                  [key intValue]);
        ...
        if (internalId != -1) {
            ...
            TGLog(@"CALL manager.incomingRequested call=%lld admin=%d participant=%d",
                  requestedContext.callId,
                  requestedContext.adminId,
                  requestedContext.participantId);
            
            bool hasActiveCall = false;
            
            for (TGCallContext *context in [_callContexts allValues])
            {
                if (requestedContext.adminId == context.session.peerId ||
                    requestedContext.participantId == context.session.peerId)
            ...
            _callContexts[@(internalId)] = context;
            TGLog(@"CALL manager.context.created internal=%d call=%lld count=%d", internalId, requestedContext.callId, (int)_callContexts.count);
            
            if (!hasActiveCall)
            {
                context.session = [[TGCallSession alloc]
                                   initWithSignal:[self callStateWithInternalId:@(internalId)]
                                   outgoing:false];
            ...
            {
                context.context = [[TGCallRequestedContext alloc]
                                   initWithCallId:requestedContext.callId
                                   accessHash:requestedContext.accessHash
                                   date:requestedContext.date
                                   adminId:requestedContext.adminId
                                   participantId:requestedContext.participantId
                                   gAHash:requestedContext.gAHash
                                   declined:true];
...
- (void)handleSignalingDataForCallId:(int64_t)callId data:(NSData *)data {
    [_queue dispatch:^{
        ...
        if (matchedContext.session != nil) {
            TGLog(@"CALL signaling.route call=%lld internal=%d dataLen=%d",
                  callId,
                  [matchedInternalId intValue],
                  (int)data.length);
            
            [matchedContext.session receiveSignalingData:data];
        }
        else {
            TGLog(@"CALL signaling.route.miss call=%lld dataLen=%d contexts=%d",
                  callId,
                  (int)data.length,
                  (int)_callContexts.count);
        }
    }];
}
...
- (void)performCallContextTransitionWithInternalId:(int32_t)internalId toCallContext:(id)toCallContext {
    ...
    if (context.context == nil) {
        if ([toCallContext isKindOfClass:[TGCallRequestedContext class]]) {
            ...
            if (![TGCallManager useCallKit] ||
                ![TGCallKitAdapter callKitAvailable] ||
                ![TGCallSession hasMicrophoneAccess] ||
                isSimulator) {
                ...
                TGDispatchOnMainThread(^{
                    [_callKitAdapter reportIncomingCallWithPeerId:requestedContext.adminId
                                                          session:context.session
                                                             uuid:context.uuid
                                                       completion:^(bool silent) {
                                                           TGDispatchOnMainThread(^{
                                                               if (silent)
                                                                   [context.session presentCallNotification:requestedContext.adminId];
                                                           });
            ...
            [self setContextState:internalId state:[[TGCallStateData alloc]
                                                    initWithInternalId:@(internalId)
                                                    callId:requestedContext.callId
                                                    accessHash:requestedContext.accessHash
                                                    state:TGCallStateHandshake
                                                    peerId:requestedContext.adminId
                                                    connection:nil
                                                    hungUpOutside:false
                                                    needsRating:false
                                                    needsDebug:false
            ...
            [context.disposable setDisposable:[[[TGCallSignals
                                                 receivedIncomingCallWithCallId:requestedContext.callId
                                                 accessHash:requestedContext.accessHash
                                                 date:requestedContext.date
                                                 adminId:requestedContext.adminId
                                                 participantId:requestedContext.participantId
                                                 gAHash:requestedContext.gAHash] deliverOn:_queue] startWithNext:^(TGCallOngoingContext *next) {
                ...
                if (strongSelf != nil) {
                    ...
                    if (rpcError != nil)
                    {
                        [strongSelf setContextState:internalId state:[[TGCallStateData alloc]
                                                                      initWithInternalId:@(internalId)
                                                                      callId:0
                                                                      accessHash:0
                                                                      state:TGCallStateEnded
        ...
        if ([context.context isKindOfClass:[TGCallDiscardedContext class]]) {
            ...
            [self setContextState:internalId state:[[TGCallStateData alloc]
                                                    initWithInternalId:@(internalId)
                                                    callId:discardedContext.callId
                                                    accessHash:0
                                                    state:callState
                                                    peerId:0
                                                    connection:nil
                                                    hungUpOutside:discardedContext.outside
                                                    needsRating:discardedContext.needsRating
                                                    needsDebug:discardedContext.needsDebug
                                                    error:discardedContext.error]];
            
            _endedIncomingCallInternalIdsPipe.sink(@(internalId));
            
            if (!context.session.hasCallKit ||
                declined ||
                context.session.completed ||
                (discardedContext.reason == TGCallDiscardReasonBusy && !discardedContext.outside)) {
                
                [self cleanupContext:internalId];
            }
            else {
                TGDispatchOnMainThread(^{
                    [_callKitAdapter endCallWithUUID:context.uuid
                                              reason:discardedContext.reason
                                          completion:^{
                                              [_queue dispatch:^{
                                                  [self cleanupContext:internalId];
                                              }];
        ...
        if ([toCallContext isKindOfClass:[TGCallWaitingContext class]]) {
            ...
            [self setContextState:internalId state:[[TGCallStateData alloc]
                                                    initWithInternalId:@(internalId)
                                                    callId:waitingContext.callId
                                                    accessHash:waitingContext.accessHash
                                                    state:TGCallStateWaiting
                                                    peerId:waitingContext.participantId
                                                    connection:nil
                                                    hungUpOutside:false
                                                    needsRating:false
                                                    needsDebug:false
        ...
        if ([toCallContext isKindOfClass:[TGCallWaitingContext class]]) {
            ...
            if (updatedWaitingContext.receiveDate > waitingContext.receiveDate) {
                TGCallWaitingContext *newWaitingContext = [[TGCallWaitingContext alloc]
                                                           initWithCallId:updatedWaitingContext.callId
                                                           accessHash:updatedWaitingContext.accessHash
                                                           date:updatedWaitingContext.date
                                                           adminId:updatedWaitingContext.adminId
                                                           participantId:updatedWaitingContext.participantId
                                                           a:waitingContext.a
                                                           gA:waitingContext.gA
                                                           dhConfig:waitingContext.dhConfig
                                                           receiveDate:updatedWaitingContext.receiveDate];
                
                context.context = newWaitingContext;
                
                [self setContextState:internalId state:[[TGCallStateData alloc]
                                                        initWithInternalId:@(internalId)
                                                        callId:newWaitingContext.callId
                                                        accessHash:newWaitingContext.accessHash
                                                        state:TGCallStateWaitingReceived
                                                        peerId:newWaitingContext.participantId
                                                        connection:nil
                                                        hungUpOutside:false
                                                        needsRating:false
                                                        needsDebug:false
            ...
            NSData *nKeyId = [[NSData alloc]
                              initWithBytes:(((uint8_t *)keyHash.bytes) + keyHash.length - 8)
                              length:8];
            
            int64_t keyId = 0;
            [nKeyId getBytes:&keyId length:8];
            
            context.context = [[TGCallAcceptedContext alloc]
                               initWithCallId:acceptedContext.callId
                               accessHash:acceptedContext.accessHash
                               date:acceptedContext.date
                               adminId:acceptedContext.adminId
                               participantId:acceptedContext.participantId
                               gA:waitingContext.gA
                               gB:acceptedContext.gB];
            
            __weak TGCallManager *weakSelf = self;
            
            [context.disposable setDisposable:[[[TGCallSignals
                                                 confirmedCallWithCallId:acceptedContext.callId
                                                 accessHash:acceptedContext.accessHash
                                                 key:key
                                                 gABytes:waitingContext.gA
                                                 keyId:keyId] deliverOn:_queue] startWithNext:^(TGCallOngoingContext *next) {
        ...
        if ([toCallContext isKindOfClass:[TGCallRequestedContext class]]) {
            ...
            [self setContextState:internalId state:[[TGCallStateData alloc]
                                                    initWithInternalId:@(internalId)
                                                    callId:waitingConfirmContext.callId
                                                    accessHash:waitingConfirmContext.accessHash
                                                    state:TGCallStateAccepting
                                                    peerId:waitingConfirmContext.adminId
                                                    connection:nil
                                                    hungUpOutside:false
                                                    needsRating:false
                                                    needsDebug:false
            ...
            [self setContextState:internalId state:[[TGCallStateData alloc]
                                                    initWithInternalId:@(internalId)
                                                    callId:receivedContext.callId
                                                    accessHash:receivedContext.accessHash
                                                    state:TGCallStateReady
                                                    peerId:receivedContext.adminId
                                                    connection:nil
                                                    hungUpOutside:false
                                                    needsRating:false
                                                    needsDebug:false
        ...
        if ([toCallContext isKindOfClass:[TGCallWaitingConfirmContext class]]) {
            TGCallWaitingConfirmContext *waitingConfirmContext = toCallContext;
            context.context = waitingConfirmContext;
            TGLog(@"CALL manager.accept.waitingConfirm internal=%d call=%lld", internalId, waitingConfirmContext.callId);
            // phone.acceptCall also carries users; re-emit the current state so
            // TGCallSession can resolve the caller name from the freshly updated DB.
            [self setContextState:internalId state:context.state];
        }
        else if ([toCallContext isKindOfClass:[TGCallConfirmedContext class]]) {
            // The server can push updatePhoneCall(phoneCall) before the
            // phone.acceptCall RPC result reaches us. The old state machine
            // treated Received -> Confirmed as invalid and destroyed the call.
            TGCallReceivedContext *receivedContext = (TGCallReceivedContext *)context.context;
            TGCallConfirmedContext *confirmedContext = (TGCallConfirmedContext *)toCallContext;
            TGLog(@"CALL manager.accept.race receivedToConfirmed internal=%d call=%lld", internalId, confirmedContext.callId);

            TGCallWaitingConfirmContext *syntheticWaiting = [[TGCallWaitingConfirmContext alloc]
                                                              initWithCallId:receivedContext.callId
                                                              accessHash:receivedContext.accessHash
                                                              date:receivedContext.date
                                                              adminId:receivedContext.adminId
                                                              participantId:receivedContext.participantId
                                                              b:receivedContext.b
                                                              gAHash:receivedContext.gAHash
                                                              dhConfig:receivedContext.dhConfig
                                                              receiveDate:0];
            context.context = syntheticWaiting;
            [self performCallContextTransitionWithInternalId:internalId toCallContext:confirmedContext];
        }
        else {
            invalidTransition(context.context, toCallContext);
        }
        ...
        if ([toCallContext isKindOfClass:[TGCallConfirmedContext class]]) {
            ...
            if (keyId == confirmedContext.keyFingerprint &&
                [gAHash isEqual:previousContext.gAHash]) {
                ...
                TGLog(@"CALL manager.confirmed.protocol call=%lld min=%d max=%d versions=%@ webrtc=%d",
                      confirmedContext.callId,
                      confirmedContext.minLayer,
                      confirmedContext.maxLayer,
                      confirmedContext.libraryVersions,
                      (int)confirmedContext.webrtcConnections.count);
                
                context.context = [[TGCallOngoingContext alloc]
                                   initWithCallId:confirmedContext.callId
                                   accessHash:confirmedContext.accessHash
                                   date:confirmedContext.date
                                   adminId:confirmedContext.adminId
                                   participantId:confirmedContext.participantId
                                   key:key
                                   keyFingerprint:keyId
                                   defaultConnection:confirmedContext.defaultConnection
                                   alternativeConnections:confirmedContext.alternativeConnections
                                   webrtcConnections:confirmedContext.webrtcConnections
                                   customParameters:confirmedContext.customParameters
                                   minLayer:confirmedContext.minLayer
                                   maxLayer:confirmedContext.maxLayer
                                   libraryVersions:confirmedContext.libraryVersions];
                
                TGCallConnection *connection = [[TGCallConnection alloc]
                                                initWithKey:key
                                                keyHash:visKeyHash
                                                defaultConnection:confirmedContext.defaultConnection
                                                alternativeConnections:confirmedContext.alternativeConnections
                ...
                [self setContextState:internalId state:[[TGCallStateData alloc]
                                                        initWithInternalId:@(internalId)
                                                        callId:confirmedContext.callId
                                                        accessHash:confirmedContext.accessHash
                                                        state:TGCallStateOngoing
                                                        peerId:confirmedContext.adminId
        ...
        if ([toCallContext isKindOfClass:[TGCallOngoingContext class]]) {
            ...
            TGLog(@"CALL manager.ongoing.protocol call=%lld min=%d max=%d versions=%@ webrtc=%d",
                  ongoingContext.callId,
                  ongoingContext.minLayer,
                  ongoingContext.maxLayer,
                  ongoingContext.libraryVersions,
                  (int)ongoingContext.webrtcConnections.count);
            
            TGCallConnection *connection = [[TGCallConnection alloc]
                                            initWithKey:ongoingContext.key
                                            keyHash:visKeyHash
                                            defaultConnection:ongoingContext.defaultConnection
                                            alternativeConnections:ongoingContext.alternativeConnections
                                            webrtcConnections:ongoingContext.webrtcConnections
                                            customParameters:ongoingContext.customParameters
                                            minLayer:ongoingContext.minLayer
                                            maxLayer:ongoingContext.maxLayer
                                            libraryVersions:ongoingContext.libraryVersions];
            
            [self setContextState:internalId state:[[TGCallStateData alloc]
                                                    initWithInternalId:@(internalId)
                                                    callId:ongoingContext.callId
                                                    accessHash:ongoingContext.accessHash
                                                    state:TGCallStateOngoing
                                                    peerId:ongoingContext.participantId
                                                    connection:connection
                                                    hungUpOutside:false
                                                    needsRating:false
                                                    needsDebug:false
        ...
        if ([toCallContext isKindOfClass:[TGCallConfirmedContext class]])
            return;
        if ([toCallContext isKindOfClass:[TGCallWaitingConfirmContext class]]) {
            TGLog(@"CALL manager.accept.staleWaitingConfirm internal=%d ignored", internalId);
            // The RPC result may be the first place where the caller's User
            // object arrives. Refresh the session without changing call state.
            [self setContextState:internalId state:context.state];
            return;
        }
...
- (void)cleanupContext:(int32_t)internalId {
    ...
    TGCallContext *context = _callContexts[@(internalId)];
    TGLog(@"CALL manager.context.cleanup internal=%d call=%lld state=%d context=%@ subscribers=%d",
...
- (void)acceptCallWithInternalId:(id)internalId {
    ...
    [[[[[self callStateWithInternalId:internalId] mapToSignal:^SSignal *(TGCallStateData *state) {
        if (state.state != TGCallStateHandshake &&
            state.state != TGCallStateReady)
        ...
        [_queue dispatch:^{
            ...
            [self setContextState:internalIdVal state:[[TGCallStateData alloc]
                                                       initWithInternalId:internalId
                                                       callId:receivedContext.callId
                                                       accessHash:receivedContext.accessHash
                                                       state:TGCallStateAccepting
            ...
            [context.disposable setDisposable:[[[TGCallSignals
                                                 acceptedIncomingCallWithCallId:receivedContext.callId
                                                 accessHash:receivedContext.accessHash
                                                 dhConfig:receivedContext.dhConfig
                                                 bBytes:receivedContext.b
                                                 gBBytes:receivedContext.gB
                                                 gAHash:receivedContext.gAHash] deliverOn:_queue] startWithNext:^(id next) {
                ...
                if (strongSelf != nil) {
                    // [SSignal fail:nil] is used by the TL adapter when an RPC
                    // succeeded but its phone.phoneCall payload could not be
                    // represented by the legacy generated class. Do not hang up
                    // a perfectly valid incoming call in that case; keep enough
                    // DH state to accept the authoritative updatePhoneCall that
                    // immediately follows from the server.
                    if (error == nil) {
                        [strongSelf->_queue dispatch:^{
                            TGCallContext *liveContext = strongSelf->_callContexts[internalId];
                            if (liveContext == nil)
                                return;
                            if ([liveContext.context isKindOfClass:[TGCallReceivedContext class]]) {
                                TGCallWaitingConfirmContext *fallback = [[TGCallWaitingConfirmContext alloc]
                                                                        initWithCallId:receivedContext.callId
                                                                        accessHash:receivedContext.accessHash
                                                                        date:receivedContext.date
                                                                        adminId:receivedContext.adminId
                                                                        ...
                                                                        dhConfig:receivedContext.dhConfig
                                                                        receiveDate:0];
                                liveContext.context = fallback;
                                TGLog(@"CALL manager.accept.parseFallback internal=%d call=%lld", internalIdVal, receivedContext.callId);
                            } else {
                                TGLog(@"CALL manager.accept.parseFallback ignored internal=%d context=%@", internalIdVal, NSStringFromClass([liveContext.context class]));
                            }
                        }];
                        return;
                    }
...
- (void)discardCallWithInternalId:(id)internalId reason:(TGCallDiscardReason)reason error:(NSString *)error {
    ...
    [_queue dispatch:^{
        ...
        TGCallDiscardReason localReason =
        ...
        {
            TGCallDiscardedContext *callContext = [[TGCallDiscardedContext alloc]
        ...
        if ([context.context isKindOfClass:[TGCallRequestingContext class]]) {
            ...
            [context.disposable setDisposable:[[[[TGCallSignals
                                                  discardedCallWithCallId:callId
                                                  accessHash:accessHash
                                                  reason:reason
                                                  duration:(int32_t)context.session.duration]
                                                 timeout:5.0
                                                 onQueue:_queue
                                                 orSignal:[SSignal single:[[TGCallDiscardedContext alloc]
                                                                           initWithCallId:callId
                                                                           reason:localReason
                                                                           outside:false
                                                                           needsRating:false
                                                                           needsDebug:false
                                                                           error:nil]]]
                                                deliverOn:_queue] startWithNext:^(TGCallDiscardedContext *next) {
                
                __strong TGCallManager *strongSelf = weakSelf;
                
                if (strongSelf != nil) {
                    if (next.reason != localReason)
                        next = [[TGCallDiscardedContext alloc]
                                initWithCallId:next.callId
                                reason:localReason
                                outside:next.outside
                                needsRating:next.needsRating
                                needsDebug:next.needsDebug
                                error:next.error];
