...
- (bool)isCurrentLifetimeToken:(id)token;

@end

static TGFileDownloadActor *TGFileDownloadActorResolve(NSString *path, id token)
{
    TGFileDownloadActor *actor = (TGFileDownloadActor *)[ActionStageInstance() executingActorWithPath:path];
    if (actor == nil || ![actor isCurrentLifetimeToken:token])
        return nil;
    return actor;
}
...
- (instancetype)initWithPath:(NSString *)path
{
    ...
    if (self != nil)
        _lifetimeToken = [[NSObject alloc] init];
    return self;
}

- (bool)isCurrentLifetimeToken:(id)token
{
    return _lifetimeToken == token;
}
    ...
    else if ([url hasPrefix:@"mt-encrypted-file://?"])
    {
        ...
        else
        {
            ...
            else
            {
                ...
                NSString *actorPath = [self.path copy];
                id lifetimeToken = _lifetimeToken;
                _workerToken = [[TGTelegramNetworking instance] requestDownloadWorkerForDatacenterId:dcId type:mediaTypeTag completion:^(TGNetworkWorkerGuard *worker)
                {
                    ...
                    {
                        TGFileDownloadActor *strongSelf = TGFileDownloadActorResolve(actorPath, lifetimeToken);
                        if (strongSelf == nil)
                            return;
                        ...
                        [request setCompleted:^(TLupload_File *response, __unused NSTimeInterval timestamp, id error)
                        {
                            [ActionStageInstance() dispatchOnStageQueue:^
                            {
                                TGFileDownloadActor *currentActor = TGFileDownloadActorResolve(actorPath, lifetimeToken);
                                if (currentActor == nil)
                                    return;
                                if (error == nil && [response isKindOfClass:[TLupload_File$upload_file class]])
                                    [currentActor filePartDownloadSuccess:location offset:0 length:(int)size data:((TLupload_File$upload_file *)response).bytes];
                                else
                                    [currentActor filePartDownloadFailed:location offset:0 length:(int)size];
                            }];
                        }];
                        ...
                        [request setProgressUpdated:^(float progress, NSUInteger packetLength)
                        {
                            [ActionStageInstance() dispatchOnStageQueue:^
                            {
                                TGFileDownloadActor *currentActor = TGFileDownloadActorResolve(actorPath, lifetimeToken);
                                if (currentActor != nil)
                                    [currentActor filePartDownloadProgress:location offset:0 length:(int)size packetLength:(int)packetLength progress:progress];
                            }];
                        }];
                ...
                if (!retrying && originInfo != nil && errorCode == 400 && [errorString hasPrefix:@"FILE_REFERENCE_"])
                {
                    [[TGDownloadMessagesSignal updatedOriginInfo:originInfo identifier:0] startWithNext:^(TGMediaOriginInfo *next)
                    {
                        [ActionStageInstance() dispatchOnStageQueue:^
                        {
                            TGFileDownloadActor *currentActor = TGFileDownloadActorResolve(actorPath, lifetimeToken);
                            if (currentActor != nil)
                                [currentActor proceedWithWorker:worker type:mediaTypeTag fileData:nil datacenterId:datacenterId volumeId:volumeId fileId:fileId secret:secret originInfo:next retrying:true];
                        }];
                    } error:^(__unused id error)
                    {
                        [ActionStageInstance() dispatchOnStageQueue:^
                        {
                            TGFileDownloadActor *currentActor = TGFileDownloadActorResolve(actorPath, lifetimeToken);
                            if (currentActor != nil)
                                [currentActor fileDownloadFailed:volumeId fileId:fileId secret:secret];
                        }];
                    } completed:nil];
                }
                else
