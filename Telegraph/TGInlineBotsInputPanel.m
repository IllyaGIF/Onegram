...
@interface TGInlineBotsInputPanel () <PSUICollectionViewDelegateFlowLayout, PSUICollectionViewDataSource> {
        ...
        if (_temporaryUser.uid == _currentBot.uid) {
            ...
            for (NSUInteger i = 0; i < _users.count; i++) {
                ...
                if (user.uid == _currentBot.uid) {
                    ...
                    [cell setFocused:true animated:animated];
                    PSUICollectionViewLayoutAttributes *attributes = [_collectionView layoutAttributesForItemAtIndexPath:[NSIndexPath indexPathForRow:i inSection:2]];
                    if (attributes != nil) {
