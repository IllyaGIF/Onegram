...
- (instancetype)initWithFrame:(CGRect)frame presentation:(TGInstantPagePresentation *)presentation autoNightThemeEnabled:(bool)autoNightThemeEnabled {
    ...
    if (self != nil) {
        ...
        if ([_autoThemeLabel respondsToSelector:@selector(setMinimumScaleFactor:)])
            _autoThemeLabel.minimumScaleFactor = 0.7f;
        else
            _autoThemeLabel.minimumFontSize = _autoThemeLabel.font.pointSize * 0.7f;
...
- (void)updatePresentation:(TGInstantPagePresentation *)presentation animated:(bool)animated {
    ...
    UIImage *panelBackground = nil;
    {
        ...
        UIEdgeInsets capInsets = UIEdgeInsetsMake(13.0f, 13.0f, 13.0f, 13.0f);
        if ([image respondsToSelector:@selector(resizableImageWithCapInsets:resizingMode:)])
            panelBackground = [image resizableImageWithCapInsets:capInsets resizingMode:UIImageResizingModeStretch];
        else
            panelBackground = [image resizableImageWithCapInsets:capInsets];
        UIGraphicsEndImageContext();
    }
