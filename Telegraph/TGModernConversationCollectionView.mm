...
- (void)layoutSubviews
{
    ...
    CGFloat relativeBoundsThreshold = cpuCoreCount() > 1 ? 5.0f : 16.0f;
    bool shouldUpdateRelativeBounds = relativeBoundsDelta > relativeBoundsThreshold;
...
- (void)setDimmed:(bool)dimmed frontCells:(NSArray *)frontCells animated:(bool)animated
{
    ...
    {
        for (PSUICollectionViewCell *cell in frontCells)
