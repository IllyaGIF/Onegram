...
@interface TGModernConversationInputTextPanel () <HPGrowingTextViewDelegate, TGModernConversationInputAttachButtonDelegate, TGModernConversationInputMicButtonDelegate>
{
    ...
    TGActivityIndicatorView *_progressButtonIndicator;
...
- (void)setPresentation:(TGPresentation *)presentation
{
    ...
    if ([TGPresentation classicIOS6Style])
    {
        ...
        if (toolbarImage != nil)
        {
            ...
            toolbarImage = [TGPresentation classicIOS6ThemedImage:toolbarImage tintColor:presentation.pallete.barBackgroundColor alpha:0.85f];
        ...
        else
        {
            ...
            UIColor *topColor = [TGPresentation classicIOS6UsesPaletteAdaptedAssets] ? [presentation.pallete.barBackgroundColor colorWithHueMultiplier:1.0f saturationMultiplier:1.0f brightnessMultiplier:(presentation.pallete.isDark ? 1.12f : 1.03f)] : UIColorRGB(0xf5f7f9);
            UIColor *bottomColor = [TGPresentation classicIOS6UsesPaletteAdaptedAssets] ? [presentation.pallete.barBackgroundColor colorWithHueMultiplier:1.0f saturationMultiplier:1.0f brightnessMultiplier:(presentation.pallete.isDark ? 0.82f : 0.84f)] : UIColorRGB(0xaeb8c2);
            _classicIOS6BackgroundGradient.colors = @[(id)topColor.CGColor, (id)bottomColor.CGColor];
        }
        if (toolbarImage == nil)
            _classicIOS6BackgroundGradient.hidden = false;
        _backgroundView.backgroundColor = [UIColor clearColor];
        _stripeView.backgroundColor = [TGPresentation classicIOS6UsesPaletteAdaptedAssets] ? presentation.pallete.barSeparatorColor : UIColorRGB(0x6d7884);
    }
    else
    {
        _classicIOS6BackgroundGradient.hidden = true;
...
- (void)growingTextViewDidChange:(HPGrowingTextView *)growingTextView afterSetText:(bool)afterSetText afterPastingText:(bool)afterPastingText
{
    ...
    NSInteger idx = (NSInteger)_inputField.internalTextView.selectedRange.location;
    idx--;
...
- (void)updateSendButtonVisibility:(bool)animated
{
    ...
    if (iosMajorVersion() < 7)
    {
        NSArray *buttons = TGIsPad() ? @[_sendButton, _micButton] : @[_sendButton, _micButton, _stickersArrowButton];
        for (UIButton *button in buttons)
        {
            ...
            bool active = button == appearingButton;
            button.userInteractionEnabled = active;
            button.alpha = active ? 1.0f : 0.0f;
    ...
    _micButton.hidden = false;
    _stickersArrowButton.hidden = false;
...
- (void)micButtonInteractionBegan
{
    ...
    if ([delegate respondsToSelector:@selector(inputPanelAudioRecordingStart:video:completion:)]) {
        if ([self videoMessageAvailable])
        {
            ...
            [self performSelector:@selector(decideMicButtonAction) withObject:nil afterDelay:(iosMajorVersion() < 7 ? 0.3 : 0.17)];
        }
        else
        {
            __weak TGModernConversationInputTextPanel *weakSelf = self;
...
- (void)setReplyMarkup:(TGBotReplyMarkup *)replyMarkup
{
    if (!TGObjectCompare(_replyMarkup, replyMarkup))
    {
        ...
        TGLog(@"BOT inputPanel setReplyMarkup rows=%d inline=%d enable=%d canAuto=%d shouldShow=%d custom=%@", (int)replyMarkup.rows.count, replyMarkup.isInline ? 1 : 0, _enableKeyboard ? 1 : 0, _canShowKeyboardAutomatically ? 1 : 0, shouldShowBotKeyboard ? 1 : 0, NSStringFromClass([_customKeyboardView class]));
...
- (void)setDisplayProgress:(bool)displayProgress {
    if (_displayProgress != displayProgress) {
        ...
        TGActivityIndicatorView *progressIndicator = _progressButtonIndicator;
        if (_displayProgress) {
            if (progressIndicator == nil) {
                progressIndicator = [[TGActivityIndicatorView alloc] initWithActivityIndicatorStyle:(UIActivityIndicatorViewStyle)4];
