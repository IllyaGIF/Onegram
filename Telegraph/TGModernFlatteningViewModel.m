...
- (void)updateSubmodelContentsForVisibleRect:(CGRect)rect
{
    ...
    if (_tiledMode && (_context == nil || !_context.contentUpdatesDisabled) && [self boundView] != nil)
    {
        ...
        {
            TGModernFlatteningView *view = (TGModernFlatteningView *)[self boundView];
            CGFloat tileHeight = TGModernFlatteningViewModelTilingLimit;
            CGSize tiledViewSize = CGSizeMake(view.frame.size.width, tileHeight);
            CGRect contentBounds = CGRectMake(0.0f, 0.0f, view.frame.size.width, view.frame.size.height);
            CGRect preloadRect = CGRectIntersection(contentBounds, CGRectInset(rect, 0.0f, -tileHeight));

            if (_tiledPartViews == nil)
                _tiledPartViews = [[NSMutableArray alloc] init];

            for (NSInteger index = (NSInteger)_tiledPartViews.count - 1; index >= 0; index--)
            {
                TGModernFlatteningView *tileView = _tiledPartViews[(NSUInteger)index];
                if (CGRectIsNull(preloadRect) || !CGRectIntersectsRect(preloadRect, tileView.frame))
                {
                    [tileView removeFromSuperview];
                    [_viewStorage enqueueView:tileView];
                    [_tiledPartViews removeObjectAtIndex:(NSUInteger)index];
                }
            }

            if (!CGRectIsNull(preloadRect) && preloadRect.size.height > FLT_EPSILON)
            {
                NSInteger firstTile = MAX((NSInteger)0, (NSInteger)floor(CGRectGetMinY(preloadRect) / tileHeight));
                NSInteger lastTile = MAX(firstTile, (NSInteger)floor((CGRectGetMaxY(preloadRect) - TGScreenPixel) / tileHeight));
                NSInteger maxTile = MAX((NSInteger)0, (NSInteger)ceil(view.frame.size.height / tileHeight) - 1);
                lastTile = MIN(lastTile, maxTile);

                for (NSInteger tileIndex = firstTile; tileIndex <= lastTile; tileIndex++)
                {
                    CGFloat originY = tileIndex * tileHeight;
                    TGModernFlatteningView *tileView = nil;

                    for (TGModernFlatteningView *currentTileView in _tiledPartViews)
                    {
                        if ((NSInteger)currentTileView.frame.origin.y == (NSInteger)originY)
                        {
                    ...
                    if (tileView == nil)
                    {
                        ...
                        tileView = (TGModernFlatteningView *)[_viewStorage dequeueViewWithIdentifier:tileViewIdentifier viewStateIdentifier:nil];
                        if (tileView == nil)
                        {
                        ...
                        tileView.frame = CGRectMake(0.0f, originY, tiledViewSize.width, MIN(tiledViewSize.height, view.frame.size.height - originY));
