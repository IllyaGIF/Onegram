...
static NSString *TGIOS6ChannelNotificationLocalized(NSString *key, NSString *fallback)
{
...
@interface TGNotificationSettingsController () <TGAlertSoundControllerDelegate>
{
    ...
    TGSwitchCollectionItem *_channelAlert;
    ...
    NSMutableDictionary *_channelNotificationSettings;
...
- (id)init
{
    ...
    if (self)
    {
        ...
        _channelNotificationSettings = [[NSMutableDictionary alloc] initWithDictionary:@{@"muteUntil": @(0), @"soundId": @(1), @"previewText": @(true)}];
        ...
        _channelAlert = [[TGSwitchCollectionItem alloc] initWithTitle:TGIOS6ChannelNotificationLocalized(@"Notifications.ChannelNotificationsAlert", @"Notifications") isOn:true];
        _channelAlert.interfaceHandle = _actionHandle;

        TGCollectionMenuSection *channelNotificationsSection = [[TGCollectionMenuSection alloc] initWithItems:@[
            [[TGHeaderCollectionItem alloc] initWithTitle:TGIOS6ChannelNotificationLocalized(@"Notifications.ChannelNotifications", @"CHANNEL NOTIFICATIONS")],
            _channelAlert,
            [[TGCommentCollectionItem alloc] initWithText:TGIOS6ChannelNotificationLocalized(@"Notifications.ChannelNotificationsHelp", @"Disable notifications from all broadcast channels. Groups and private chats are not affected.")]
        ]];
        [self.menuSections addSection:channelNotificationsSection];
        ...
        [ActionStageInstance() dispatchOnStageQueue:^
        {
            [ActionStageInstance() watchForPaths:@[
                ...
                [NSString stringWithFormat:@"/tg/peerSettings/(%d)", INT_MAX - 4]
            ] watcher:self];
            ...
            [ActionStageInstance() requestActor:[NSString stringWithFormat:@"/tg/peerSettings/(%d,cached)", INT_MAX - 4] options:[NSDictionary dictionaryWithObject:[NSNumber numberWithLongLong:INT_MAX - 4] forKey:@"peerId"] watcher:self];
        }];
...
- (void)_updateItems:(bool)animated
{
    ...
    [_channelAlert setIsOn:[[_channelNotificationSettings objectForKey:@"muteUntil"] intValue] == 0 animated:animated];
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    if ([action isEqualToString:@"switchItemChanged"])
    {
        ...
        else if (switchItem == _channelAlert)
        {
            int muteUntil = switchItem.isOn ? 0 : INT_MAX;
            _channelNotificationSettings[@"muteUntil"] = @(muteUntil);

            TGLog(@"NOTIFY channels.switch enabled=%d muteUntil=%d", switchItem.isOn ? 1 : 0, muteUntil);

            static int actionId = 0;
            [ActionStageInstance() requestActor:[NSString stringWithFormat:@"/tg/changePeerSettings/(%d)/(pc%d)", INT_MAX - 4, actionId++] options:@{
                @"peerId": @(INT_MAX - 4),
                @"muteUntil": @(muteUntil)
            } watcher:TGTelegraphInstance];
        }
    ...
    if ([path hasPrefix:@"/tg/peerSettings/"])
    {
        if (resultCode == ASStatusSuccess)
        {
            ...
            TGDispatchOnMainThread(^
            {
                ...
                else if ([path hasPrefix:[NSString stringWithFormat:@"/tg/peerSettings/(%d", INT_MAX - 4]])
                {
                    _channelNotificationSettings = [notificationSettings mutableCopy];
                    TGLog(@"NOTIFY channels.loaded muteUntil=%@", _channelNotificationSettings[@"muteUntil"]);
                    [self _updateItems:false];
                }
            });
        }
