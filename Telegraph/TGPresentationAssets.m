...
+ (UIImage *)tabBarContactsIcon:(UIColor *)color
{
    if (TGClassicIOS6StyleEnabled())
    {
        ...
        if (classicImage != nil)
            return [TGPresentation classicIOS6ThemedImage:classicImage tintColor:color alpha:0.55f];
    }
...
+ (UIImage *)premiumBadgeIcon:(CGFloat)diameter
{
    if (diameter < 1.0f)
        diameter = 14.0f;

    static UIImage *dialogBadge = nil;
    static UIImage *profileBadge = nil;
    if (diameter == 14.0f && dialogBadge != nil)
        return dialogBadge;
    if (diameter == 16.0f && profileBadge != nil)
        return profileBadge;
    ...
    CGFloat outerRadius = diameter / 2.0f - 0.5f;
    CGFloat innerRadius = outerRadius * 0.47f;

    UIBezierPath *starPath = [UIBezierPath bezierPath];
    for (NSInteger i = 0; i < 10; i++)
    {
        CGFloat radius = (i % 2 == 0) ? outerRadius : innerRadius;
        CGFloat angle = -((CGFloat)M_PI / 2.0f) + ((CGFloat)i * (CGFloat)M_PI / 5.0f);
    ...
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    CGFloat locations[3] = {0.0f, 0.55f, 1.0f};
    CGFloat components[12] = {
        0.33f, 0.67f, 1.00f, 1.0f,
        0.48f, 0.43f, 0.98f, 1.0f,
        0.76f, 0.31f, 0.91f, 1.0f
    };
    CGGradientRef gradient = CGGradientCreateWithColorComponents(colorSpace, components, locations, 3);
    CGContextDrawLinearGradient(context, gradient, CGPointMake(0.0f, 0.0f), CGPointMake(diameter, diameter), 0);
...
+ (UIImage *)inputPanelFieldBackground:(UIColor *)color borderColor:(UIColor *)borderColor
{
    if (TGClassicIOS6StyleEnabled())
    {
        ...
        if (classicImage != nil)
        {
            UIImage *adaptedImage = [TGPresentation classicIOS6ThemedImage:classicImage tintColor:color alpha:0.82f];
            return [adaptedImage stretchableImageWithLeftCapWidth:14 topCapHeight:13];
        }
    }
...
+ (UIImage *)menuCornersImage:(UIColor *)color
{
    ...
    UIImage *cornersImage = [UIGraphicsGetImageFromCurrentImageContext() stretchableImageWithLeftCapWidth:(NSInteger)radius topCapHeight:(NSInteger)radius];
    UIGraphicsEndImageContext();
