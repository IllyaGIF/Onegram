...
- (SSignal *)_requestPinnedMessages
{
    ...
    {
        TGLog(@"TRACE pinned.search.error peer=%lld filter=inputMessagesFilterPinned scope=private error=%@", peerId, error);
...
- (void)_createOrUpdatePrimaryTitlePanel:(bool)createIfNeeded
{
    ...
    TGLog(@"CALL ui.titlePanel peer=%d supports=%d private=%d forceShow=1", _uid, _supportsCalls ? 1 : 0, _callsPrivate ? 1 : 0);
...
- (void)startVoiceCall {
    TGLog(@"CALL ui.startVoiceCall peer=%d supports=%d private=%d", _uid, _supportsCalls ? 1 : 0, _callsPrivate ? 1 : 0);
    if (_callsPrivate) {
        TGUser *user = [TGDatabaseInstance() loadUser:_uid];
