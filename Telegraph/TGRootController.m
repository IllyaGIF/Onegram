...
@interface TGRootController ()
{
    ...
    bool _ios4ManualContainerAppeared;
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    {
        if (_masterNavigationController.view.superview != nil)
            [_masterNavigationController viewWillAppear:animated];
        if (_detailNavigationController.view.superview != nil)
            [_detailNavigationController viewWillAppear:animated];
    }
}
...
- (void)viewDidAppear:(BOOL)animated
{
    ...
    {
        if (_masterNavigationController.view.superview != nil)
            [_masterNavigationController viewDidAppear:animated];
        if (_detailNavigationController.view.superview != nil)
            [_detailNavigationController viewDidAppear:animated];
        _ios4ManualContainerAppeared = true;
    }
}
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    {
        if (_masterNavigationController.view.superview != nil)
            [_masterNavigationController viewWillDisappear:animated];
        if (_detailNavigationController.view.superview != nil)
            [_detailNavigationController viewWillDisappear:animated];
    }
...
- (void)viewDidDisappear:(BOOL)animated
{
    ...
    {
        if (_masterNavigationController.view.superview != nil)
            [_masterNavigationController viewDidDisappear:animated];
        if (_detailNavigationController.view.superview != nil)
            [_detailNavigationController viewDidDisappear:animated];
        _ios4ManualContainerAppeared = false;
    }
...
- (void)setPresentation:(TGPresentation *)presentation
{
    ...
    [_masterNavigationController setNavigationBarPallete:presentation.navigationBarPallete];
    [_detailNavigationController setNavigationBarPallete:presentation.navigationBarPallete];
    ...
    if ([self.presentedViewController isKindOfClass:[TGNavigationController class]])
    {
        ...
        [navController setNavigationBarPallete:presentation.navigationBarPallete];
...
- (bool)shouldAutorotate {
    ...
    if (self.associatedWindowStack.count > 0)
        controller = [self.associatedWindowStack.lastObject rootViewController];
    else
        controller = (UIViewController *)[self viewControllers].lastObject;

    if (controller == nil)
        return true;
    if ([controller respondsToSelector:@selector(shouldAutorotate)])
        return [controller shouldAutorotate];
    return [controller shouldAutorotateToInterfaceOrientation:[UIApplication sharedApplication].statusBarOrientation];
}
...
- (void)removeDetailController {
    ...
    } else if ([_detailNavigationController isViewLoaded] && _detailNavigationController.view.superview != nil) {
        if (_ios4ManualContainerAppeared)
            [_detailNavigationController viewWillDisappear:false];
        [_mainView setDetailView:nil];
        if (_ios4ManualContainerAppeared)
            [_detailNavigationController viewDidDisappear:false];
    }
}
...
- (void)addDetailController {
    if (iosMajorVersion() >= 5) {
        ...
        if (_ios4ManualContainerAppeared)
            [_detailNavigationController viewWillAppear:false];
        [_mainView setDetailView:_detailNavigationController.view];
        if (_ios4ManualContainerAppeared)
            [_detailNavigationController viewDidAppear:false];
    }
}
...
- (void)removeMasterController {
    ...
    } else if ([_masterNavigationController isViewLoaded] && _masterNavigationController.view.superview != nil) {
        if (_ios4ManualContainerAppeared)
            [_masterNavigationController viewWillDisappear:false];
        [_mainView setMasterView:nil];
        if (_ios4ManualContainerAppeared)
            [_masterNavigationController viewDidDisappear:false];
    }
}
...
- (void)addMasterController {
    if (iosMajorVersion() >= 5) {
        ...
        if (_ios4ManualContainerAppeared)
            [_masterNavigationController viewWillAppear:false];
        [_mainView setMasterView:_masterNavigationController.view];
        if (_ios4ManualContainerAppeared)
            [_masterNavigationController viewDidAppear:false];
    }
}
