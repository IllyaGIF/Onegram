...
@interface TGStickerKeyboardCollectionView : PSUICollectionView
...
@interface TGStickerKeyboardView () <PSUICollectionViewDataSource, PSUICollectionViewDelegateFlowLayout, TGGifKeyboardBalancedLayoutDelegate, UIGestureRecognizerDelegate, TGSearchBarDelegate>
{
    bool _didScroll;
    ...
    PSUICollectionView *_gifsCollectionView;
    TGGifKeyboardBalancedLayout *_gifsCollectionLayout;
    
    PSUICollectionView *_trendingCollectionView;
    PSUICollectionViewFlowLayout *_trendingCollectionLayout;
    
    TGStickerKeyboardCollectionView *_collectionView;
    PSUICollectionViewFlowLayout *_collectionLayout;
    id _visibleCollectionReusableHeaderViews;
    ...
    PSUICollectionView *_searchCollectionView;
    PSUICollectionViewFlowLayout *_searchCollectionLayout;
    ...
    id _searchVisibleCollectionReusableHeaderViews;
...
- (instancetype)initWithFrame:(CGRect)frame style:(TGStickerKeyboardViewStyle)style
{
    ...
    if (self != nil)
    {
        ...
        _collectionLayout = [[PSUICollectionViewFlowLayout alloc] init];
        ...
        [_collectionView registerClass:[TGStickerCollectionHeader class] forSupplementaryViewOfKind:PSTCollectionElementKindSectionHeader withReuseIdentifier:@"TGStickerCollectionHeader"];
        [self addSubview:_collectionView];
        _visibleCollectionReusableHeaderViews = TGWeakValueMapTable();
    
        _trendingCollectionLayout = [[PSUICollectionViewFlowLayout alloc] init];
        _trendingCollectionView = [[PSUICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_trendingCollectionLayout];
        ...
        _gifsCollectionView = [[PSUICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_gifsCollectionLayout];
        ...
        if (_style == TGStickerKeyboardViewDefaultStyle)
        {
            ...
            _searchCollectionLayout = [[PSUICollectionViewFlowLayout alloc] init];
            _searchCollectionView = [[PSUICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:_searchCollectionLayout];
            ...
            [_searchCollectionView registerClass:[TGStickerCollectionHeader class] forSupplementaryViewOfKind:PSTCollectionElementKindSectionHeader withReuseIdentifier:@"TGStickerCollectionHeader"];
            [_searchWrapperView addSubview:_searchCollectionView];
            _searchVisibleCollectionReusableHeaderViews = TGWeakValueMapTable();
...
- (void)setPresentation:(TGPresentation *)presentation
{
    ...
    for (PSUICollectionViewCell *cell in _collectionView.visibleCells)
    {
        if ([cell respondsToSelector:@selector(setPresentation:)])
            [cell performSelector:@selector(setPresentation:) withObject:presentation];
    }
    
    for (PSUICollectionViewCell *cell in _trendingCollectionView.visibleCells)
    {
        if ([cell respondsToSelector:@selector(setPresentation:)])
            [cell performSelector:@selector(setPresentation:) withObject:presentation];
    }
    
    for (PSUICollectionViewCell *cell in _searchCollectionView.visibleCells)
    {
        if ([cell respondsToSelector:@selector(setPresentation:)])
            [cell performSelector:@selector(setPresentation:) withObject:presentation];
    }
...
- (void)setupSignals
{
    ...
    _stickerPacksDisposable = [[combinedSignal deliverOn:[SQueue mainQueue]] startWithNext:^(NSArray *combinedResult)
    {
        ...
        IOS6_NOOP_LOG(@"STICKER keyboard.combined rawPacks=%d featured=%d recent=%d favorite=%d gifs=%d",
            (int)rawPacks.count,
        ...
        IOS6_NOOP_LOG(@"STICKER keyboard.filtered packs=%d recent=%d favorite=%d group=%d groupRef=%@",
            (int)reversed.count,
        ...
        if (strongSelf != nil) {
            ...
            if (![strongSelf->_stickerPacks isEqual:reversed] || ![strongSelf->_recentStickersOriginal isEqual:reversedRecentStickers] || ![strongSelf->_favoriteDocuments isEqual:reversedFavoriteStickers] || ![strongSelf->_groupDocuments isEqual:groupStickers]) {
                ...
                IOS6_NOOP_LOG(@"STICKER keyboard.set packs=%d recent=%d favorite=%d group=%d peer=%lld admin=%d groupUnpinned=%d",
                    (int)reversed.count,
            ...
            if (![strongSelf->_trendingStickerPacks isEqualToArray:trendingPacks]) {
                IOS6_NOOP_LOG(@"STICKER keyboard.trending packs=%d installed=%d unread=%d", (int)trendingPacks.count, (int)installedPacks.count, (int)unreadPacks.count);
...
- (CGFloat)collectionView:(PSUICollectionView *)collectionView layout:(PSUICollectionViewLayout*)__unused collectionViewLayout minimumLineSpacingForSectionAtIndex:(NSInteger)__unused section
{
    if (collectionView == _gifsCollectionView) {
        return 0.0f;
    } else if (collectionView == _trendingCollectionView) {
...
- (CGFloat)collectionView:(PSUICollectionView *)collectionView layout:(PSUICollectionViewLayout*)__unused collectionViewLayout minimumInteritemSpacingForSectionAtIndex:(NSInteger)__unused section
{
    if (collectionView == _gifsCollectionView) {
        return 0.0f;
    } else if (collectionView == _trendingCollectionView) {
...
- (void)collectionView:(PSUICollectionView *)collectionView didEndDisplayingCell:(PSUICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)__unused indexPath {
    if (collectionView == _gifsCollectionView) {
...
- (void)updateCurrentSection {
    if (_mode == TGStickerKeyboardViewModeGifs) {
        ...
        for (PSUICollectionViewLayoutAttributes *attributes in layoutAttributes)
        {
...
{
    if (collectionView == _searchCollectionView && [kind isEqualToString:PSTCollectionElementKindSectionHeader])
    {
        if (indexPath.section < (NSInteger)_searchLocalStickerPacks.count)
        {
            TGStickerCollectionHeader *view = [collectionView dequeueReusableSupplementaryViewOfKind:PSTCollectionElementKindSectionHeader withReuseIdentifier:@"TGStickerCollectionHeader" forIndexPath:indexPath];
    ...
    else if (collectionView == _collectionView && [kind isEqualToString:PSTCollectionElementKindSectionHeader])
    {
        if (indexPath.section == 0)
            return nil;
...
- (CGSize)collectionView:(PSUICollectionView *)collectionView layout:(PSUICollectionViewLayout *)__unused collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section
{
...
- (void)collectionView:(PSUICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    if (collectionView == _gifsCollectionView) {
        ...
        if (cell != nil && _gifSelected) {
            ...
            if (document == nil)
                return;
            IOS6_NOOP_LOG(@"STICKER keyboard.selectGif section=%d row=%d %@", (int)indexPath.section, (int)indexPath.row, TGIOS6StickerDocumentDescription(document));
        ...
        if (_searchLocalStickerPacks.count > 0 && indexPath.section != (NSInteger)_searchLocalStickerPacks.count) {
            ...
            if ([cell isEnabled])
            {
                ...
                if (document == nil)
                    return;
                IOS6_NOOP_LOG(@"STICKER keyboard.selectSearch section=%d row=%d %@", (int)indexPath.section, (int)indexPath.item, TGIOS6StickerDocumentDescription(document));
                if (_stickerSelected)
        ...
        if ([cell isEnabled])
        {
            ...
            if (document == nil)
                return;
            IOS6_NOOP_LOG(@"STICKER keyboard.select section=%d row=%d %@", (int)indexPath.section, (int)indexPath.item, TGIOS6StickerDocumentDescription(document));
            if (_stickerSelected)
                _stickerSelected(document);
        }
    }
...
- (void)scrollToSection:(NSUInteger)section fromGifs:(bool)fromGifs animated:(bool)animated
{
    ...
    else if (section == 1)
    {
        ...
        {
            PSUICollectionViewLayoutAttributes *attributes = [_collectionView layoutAttributesForItemAtIndexPath:[NSIndexPath indexPathForItem:0 inSection:section]];
...
- (void)handleSearchStickerPress:(UILongPressGestureRecognizer *)recognizer
{
    if (recognizer.state == UIGestureRecognizerStateBegan)
    {
        ...
        {
            PSUICollectionViewCell *baseCell = (TGStickerCollectionViewCell *)[_searchCollectionView cellForItemAtIndexPath:indexPath];
...
- (void)handleSearchStickerPan:(UIPanGestureRecognizer *)gestureRecognizer
{
    if (_previewController != nil && gestureRecognizer.state == UIGestureRecognizerStateChanged)
    {
        ...
        if (CGRectContainsPoint(CGRectOffset(_searchCollectionView.frame, 0, preloadInset), relativePoint))
        {
            for (NSIndexPath *indexPath in [_searchCollectionView indexPathsForVisibleItems])
            {
                PSUICollectionViewCell *baseCell = [_searchCollectionView cellForItemAtIndexPath:indexPath];
                if ([baseCell isKindOfClass:[TGTrendingStickerPackKeyboardCell class]])
                {
                    TGTrendingStickerPackKeyboardCell *trendingCell = (TGTrendingStickerPackKeyboardCell *)baseCell;
                    if (CGRectContainsPoint(trendingCell.frame, point))
...
- (CGSize)collectionView:(PSUICollectionView *)collectionView layout:(TGGifKeyboardBalancedLayout *)__unused collectionViewLayout preferredSizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (collectionView == _gifsCollectionView) {
...
- (void)installStickerPack:(TGStickerPack *)stickerPack {
    IOS6_NOOP_LOG(@"STICKER keyboard.installPack title=%@ docs=%d ref=%@",
        stickerPack.title,
    ...
    [[[installStickerPackAndGetArchivedSignal deliverOn:[SQueue mainQueue]] onDispose:^{
        ...
        IOS6_NOOP_LOG(@"STICKER keyboard.installPack.done title=%@ archived=%d", stickerPack.title, (int)archivedPacks.count);
...
{
    IOS6_NOOP_LOG(@"STICKER keyboard.viewPack title=%@ docs=%d recent=%d sticker=%@",
        stickerPack.title,
        (int)stickerPack.documents.count,
        recent ? 1 : 0,
...
- (void)previewStickerPack:(TGStickerPack *)stickerPack sticker:(TGDocumentMediaAttachment *)sticker {
    TGViewController *parentViewController = _parentViewController;
    IOS6_NOOP_LOG(@"STICKER keyboard.previewPack title=%@ docs=%d packRef=%@ sticker=%@ parent=%@",
        stickerPack.title,
        (int)stickerPack.documents.count,
        TGIOS6StickerPackReferenceDescription(stickerPack.packReference),
        TGIOS6StickerDocumentDescription(sticker),
    ...
    IOS6_NOOP_LOG(@"STICKER keyboard.previewPack.present menuPackRef=%@ expanded=%d search=%d", TGIOS6StickerPackReferenceDescription(packReference), _expanded ? 1 : 0, _searchActive ? 1 : 0);
    [TGStickersMenu presentWithParentController:parentViewController packReference:packReference stickerPack:stickerPack showShareAction:false sendSticker:^(TGDocumentMediaAttachment *document) {
        IOS6_NOOP_LOG(@"STICKER keyboard.previewPack.send %@", TGIOS6StickerDocumentDescription(document));
        __strong TGStickerKeyboardView *strongSelf = weakSelf;
        if (strongSelf != nil) {
...
- (void)collectionView:(PSUICollectionView *)collectionView willDisplayCell:(PSUICollectionViewCell *)__unused cell forItemAtIndexPath:(NSIndexPath *)indexPath {
