...
@interface TGStickersCollectionItemView () <PSUICollectionViewDataSource, PSUICollectionViewDelegate, PSUICollectionViewDelegateFlowLayout, UIGestureRecognizerDelegate, ASWatcher>
{
    TGStickerPack *_stickerPack;
    
    TGStickersCollectionView *_collectionView;
    PSUICollectionViewFlowLayout *_collectionViewLayout;
    TGScrollIndicatorView *_scrollIndicator;
    
    UIView *_separator;
    
    TGActivityIndicatorView *_activityIndicator;
    UILabel *_titleLabel;
    NSArray *_titleLinks;
...
- (instancetype)init
{
    ...
    if (self != nil)
    {
        ...
        _collectionViewLayout = [[PSUICollectionViewFlowLayout alloc] init];
...
- (void)collectionView:(PSUICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    PSUICollectionViewCell *cell = [collectionView cellForItemAtIndexPath:indexPath];
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    if ([action isEqualToString:@"menuAction"])
    {
        ...
        if ([menuAction isEqualToString:@"send"])
        {
            ...
            {
                IOS6_NOOP_LOG(@"STICKER menu.send invalidIndex=%d count=%d", (int)index, (int)_stickerPack.documents.count);
                return;
            }
            TGDocumentMediaAttachment *sticker = _stickerPack.documents[index];
            IOS6_NOOP_LOG(@"STICKER menu.send index=%d doc=%lld access=%lld dc=%d size=%d mime=%@", (int)index, (long long)sticker.documentId, (long long)sticker.accessHash, sticker.datacenterId, sticker.size, sticker.mimeType);
            if (self.sendSticker != nil)
        ...
        else if ([menuAction isEqualToString:@"faveSticker"] || [menuAction isEqualToString:@"unfaveSticker"])
        {
            ...
            if (index < 0 || index >= (NSInteger)_stickerPack.documents.count)
            {
                IOS6_NOOP_LOG(@"STICKER menu.fave invalidIndex=%d count=%d action=%@", (int)index, (int)_stickerPack.documents.count, menuAction);
                return;
            }
            TGDocumentMediaAttachment *sticker = _stickerPack.documents[index];
