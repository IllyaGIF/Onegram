...
static NSString *TGIOS6UserProfileMusicTitle(TGDocumentMediaAttachment *document)
{
    if (document == nil)
        return nil;
...
- (instancetype)initWithUid:(int32_t)uid withoutActions:(bool)withoutActions sharedMediaPeerId:(int64_t)sharedMediaPeerId sharedMediaOptions:(NSDictionary *)sharedMediaOptions
{
    ...
    if (self != nil)
    {
        ...
        // at least one visible track, so it appears immediately below the
        // avatar/name/activity card and before the phone section.
...
- (void)_reloadProfileSavedMusic
{
    ...
    _updatedCachedDataDisposable = [[[[TGUserSignal profileSavedMusicWithUserId:_uid] deliverOn:[SQueue mainQueue]] take:1] startWithNext:^(id value)
    {
        ...
        TGDocumentMediaAttachment *oldDocument = strongSelf->_profileMusicDocuments.count != 0 ? strongSelf->_profileMusicDocuments[0] : nil;
        ...
        for (NSUInteger i = 0; i < sections.count; i++)
        {
            ...
            NSUInteger index = [section.items indexOfObject:strongSelf.userInfoItem];
            if (index != NSNotFound)
            {
                profileSectionIndex = i;
                profileItemIndex = index;
                break;
            }
        }

        if (profileSectionIndex != NSNotFound)
        {
            TGCollectionMenuSection *profileSection = sections[profileSectionIndex];
            NSUInteger currentMusicIndex = [profileSection.items indexOfObject:strongSelf->_profileMusicItem];
            bool hadMusic = currentMusicIndex != NSNotFound;
            bool hasMusic = document != nil;

            if (hasMusic)
                strongSelf->_profileMusicItem.title = TGIOS6UserProfileMusicTitle(document);

            if (!hadMusic && hasMusic)
            {
                [strongSelf.menuSections beginRecordingChanges];
                [strongSelf.menuSections insertItem:strongSelf->_profileMusicItem toSection:profileSectionIndex atIndex:MIN(profileItemIndex + 1, profileSection.items.count)];
                [strongSelf.menuSections commitRecordedChanges:strongSelf.collectionView];
            }
            else if (hadMusic && !hasMusic)
            {
                [strongSelf.menuSections beginRecordingChanges];
                [strongSelf.menuSections deleteItemFromSection:profileSectionIndex atIndex:currentMusicIndex];
                [strongSelf.menuSections commitRecordedChanges:strongSelf.collectionView];
            }
            else if (hasMusic && oldDocument.documentId != document.documentId)
            {
                strongSelf->_profileMusicItem.title = TGIOS6UserProfileMusicTitle(document);
            }
        }
...
{
    TGDocumentMediaAttachment *document = _profileMusicDocuments.count != 0 ? _profileMusicDocuments[0] : nil;
...
- (void)_playProfileMusicDocument:(TGDocumentMediaAttachment *)document
{
    ...
    [[[[TGTelegraphInstance.musicPlayer playingStatus] take:1] deliverOn:[SQueue mainQueue]] startWithNext:^(TGMusicPlayerStatus *status)
    {
        ...
        [TGTelegraphInstance.musicPlayer setPlaylist:[SSignal single:playlist] initialItemKey:item.key metadata:@{ @"profileMusicPeerId": @(strongSelf->_uid), @"profileSavedMusic": @true }];
    }];
}
...
- (id)_externalShareItemForUser:(TGUser *)user
{
    ...
    else
    {
        ...
        contactData = TGVCardDataFromPerson(contact);
        if (phoneNumberMultiValue != NULL)
            CFRelease(phoneNumberMultiValue);
        if (contact != NULL)
            CFRelease(contact);
...
- (TGModernGalleryController *)createAvatarGalleryControllerForPreviewMode:(bool)previewMode
{
    ...
    if (user.photoUrlSmall.length != 0 || user.photoFullUrlSmall.length != 0)
    {
        ...
        while (false) TGLog(@"PROFILE avatar.gallery.open uid=%d small=%@ big=%@ fullSmall=%@ fullBig=%@ chosenThumb=%@ chosen=%@ hasImage=%d",
              (int)_uid,
              user.photoUrlSmall,
              user.photoUrlBig,
...
- (void)actionStageActionRequested:(NSString *)action options:(id)options
{
    ...
    else if ([action isEqualToString:@"profileMusicTapped"])
    {
        [self _playProfileMusicDocument:[options isKindOfClass:[TGDocumentMediaAttachment class]] ? options : nil];
    }
