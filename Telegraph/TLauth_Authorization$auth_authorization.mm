...
- (id<TLObject>)TLdeserialize:(NSInputStream *)is signature:(int32_t)__unused signature environment:(id<TLSerializationEnvironment>)__unused environment context:(TLSerializationContext *)__unused context error:(__autoreleasing NSError **)__unused error
{
    ...
    result.flags = flags;
    TGLog(@"AUTH auth.authorization parse signature=0x%08x flags=%d", signature, flags);
    
    if (flags & (1 << 0)) {
        result.tmp_sessions = [is readInt32];
        TGLog(@"AUTH auth.authorization tmp_sessions=%d", result.tmp_sessions);
    }
    
    if (flags & (1 << 1)) {
        int32_t otherwiseReloginDays = [is readInt32];
        TGLog(@"AUTH auth.authorization otherwise_relogin_days=%d", otherwiseReloginDays);
    }
    
    if (flags & (1 << 2)) {
        NSData *futureAuthToken = [is readBytes];
        TGLog(@"AUTH auth.authorization future_auth_token_len=%d", (int)futureAuthToken.length);
    }
    
    {
        int32_t signature = [is readInt32];
        TGLog(@"AUTH auth.authorization user signature=0x%08x", signature);
