...
static void ASQueueIOS6PerfMaybeLog(const char *name, id queue, bool synchronous, CFAbsoluteTime elapsedMs)
{
    ...
    while (false) NSLog(@"PERF slowASQueue %@ queue=%s ptr=%p sync=%d ms=%.1f skipped=%d", ASQueueIOS6PerfBuildTag(), name != NULL ? name : "main", queue, synchronous ? 1 : 0, elapsedMs, skippedNow);
