...
typedef std::tr1::shared_ptr<POPAnimatorItem> POPAnimatorItemRef;
typedef std::tr1::shared_ptr<const POPAnimatorItem> POPAnimatorItemConstRef;
...
static void POPPendingListRunLoopObserverCallback(CFRunLoopObserverRef observer, CFRunLoopActivity activity, void *info)
{
...
- (void)_scheduleProcessPendingList
{
  ...
  if (!_pendingListObserver) {
    ...
    _pendingListObserver = CFRunLoopObserverCreate(kCFAllocatorDefault, kCFRunLoopBeforeWaiting | kCFRunLoopExit, false, POPAnimationApplyRunLoopOrder, POPPendingListRunLoopObserverCallback, &context);
...
- (void)addAnimation:(POPAnimation *)anim forObject:(id)obj key:(NSString *)key
{
  ...
  if (!key) {
    key = [NSString stringWithFormat:@"%f-%u", CACurrentMediaTime(), arc4random()];
  }
...
- (void)removeAllAnimationsForObject:(id)obj
{
  ...
  NSMutableSet *animationSet = [[NSMutableSet alloc] initWithCapacity:animations.count];
