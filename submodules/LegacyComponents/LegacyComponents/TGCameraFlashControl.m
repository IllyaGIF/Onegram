...
static UIView *TGCameraFlashSnapshotView(UIView *view)
{
    if (view == nil)
        return nil;
    
    if (iosMajorVersion() >= 7 && [view respondsToSelector:@selector(snapshotViewAfterScreenUpdates:)])
        return [view snapshotViewAfterScreenUpdates:false];
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self != nil)
    {
        ...
        {
            [_autoButton setAttributedTitle:[[NSAttributedString alloc] initWithString:TGLocalized(@"Camera.FlashAuto")
                                                                            attributes:@{
                                                        NSForegroundColorAttributeName: [TGCameraInterfaceAssets normalColor],
                                                                   NSKernAttributeName: @2
                                             }] forState:UIControlStateNormal];
            
            [_autoButton setAttributedTitle:[[NSAttributedString alloc] initWithString:TGLocalized(@"Camera.FlashAuto")
                                                                            attributes:@{
                                                        NSForegroundColorAttributeName: [TGCameraInterfaceAssets accentColor],
                                                                   NSKernAttributeName: @2
                                             }] forState:UIControlStateSelected];
            
            [_autoButton setAttributedTitle:[_autoButton attributedTitleForState:UIControlStateSelected]
                                   forState:UIControlStateHighlighted | UIControlStateSelected];
            
            _autoButton.frame = (CGRect)
            {
                CGPointZero,
                [TGCameraFlashControl _sizeForModeButtonWithTitle:[_autoButton attributedTitleForState:UIControlStateNormal]]
            };
        }
        else
        {
            [_autoButton setTitle:TGLocalized(@"Camera.FlashAuto") forState:UIControlStateNormal];
            [_autoButton setTitleColor:[TGCameraInterfaceAssets normalColor] forState:UIControlStateNormal];
            [_autoButton setTitleColor:[TGCameraInterfaceAssets accentColor] forState:UIControlStateSelected];
            [_autoButton setTitleColor:[TGCameraInterfaceAssets accentColor]
                              forState:UIControlStateHighlighted | UIControlStateSelected];
        ...
        if (iosMajorVersion() >= 6)
        {
            [_onButton setAttributedTitle:[[NSAttributedString alloc] initWithString:TGLocalized(@"Camera.FlashOn")
                                                                          attributes:@{
                                                      NSForegroundColorAttributeName: [TGCameraInterfaceAssets normalColor],
                                                                 NSKernAttributeName: @2
                                           }] forState:UIControlStateNormal];
            ...
            [_onButton setAttributedTitle:[_onButton attributedTitleForState:UIControlStateSelected]
                                 forState:UIControlStateHighlighted | UIControlStateSelected];
            
            _onButton.frame = (CGRect)
            {
                CGPointZero,
                [TGCameraFlashControl _sizeForModeButtonWithTitle:[_onButton attributedTitleForState:UIControlStateNormal]]
            };
        }
        else
        {
            [_onButton setTitle:TGLocalized(@"Camera.FlashOn") forState:UIControlStateNormal];
            [_onButton setTitleColor:[TGCameraInterfaceAssets normalColor] forState:UIControlStateNormal];
            [_onButton setTitleColor:[TGCameraInterfaceAssets accentColor] forState:UIControlStateSelected];
            [_onButton setTitleColor:[TGCameraInterfaceAssets accentColor]
                            forState:UIControlStateHighlighted | UIControlStateSelected];
            [_onButton sizeToFit];
        }
        ...
        if (iosMajorVersion() >= 6)
        {
            [_offButton setAttributedTitle:[[NSAttributedString alloc] initWithString:TGLocalized(@"Camera.FlashOff")
                                                                           attributes:@{
                                                       NSForegroundColorAttributeName: [TGCameraInterfaceAssets normalColor],
                                                                  NSKernAttributeName: @2
                                            }] forState:UIControlStateNormal];
            ...
            [_offButton setAttributedTitle:[_offButton attributedTitleForState:UIControlStateSelected]
                                  forState:UIControlStateHighlighted | UIControlStateSelected];
            
            _offButton.frame = (CGRect)
            {
                CGPointZero,
                [TGCameraFlashControl _sizeForModeButtonWithTitle:[_offButton attributedTitleForState:UIControlStateNormal]]
            };
        }
        else
        {
            [_offButton setTitle:TGLocalized(@"Camera.FlashOff") forState:UIControlStateNormal];
            [_offButton setTitleColor:[TGCameraInterfaceAssets normalColor] forState:UIControlStateNormal];
            [_offButton setTitleColor:[TGCameraInterfaceAssets accentColor] forState:UIControlStateSelected];
            [_offButton setTitleColor:[TGCameraInterfaceAssets accentColor]
                             forState:UIControlStateHighlighted | UIControlStateSelected];
    ...
    if (animated)
    {
        ...
        if (active)
        {
            ...
            else
            {
                ...
                snapshotView = TGCameraFlashSnapshotView(animatedView);
            }
            ...
            {
                iconSnapshotView = TGCameraFlashSnapshotView(_flashIconView);
        ...
        else
        {
            ...
            [UIView animateWithDuration:0.25f
                             ...
                             animations:^
             {
                 ...
                 animatedView.alpha = 1.0f;
                 animatedView.frame = [self _selectedButtonFrameForSize:animatedView.frame.size
        ...
        else
        {
            switch (self.mode)
            {
                case PGCameraFlashModeOff:
                {
                    ...
                    _offButton.frame = [self _selectedButtonFrameForSize:_offButton.frame.size
                                                    interfaceOrientation:_interfaceOrientation];
                    _offButton.alpha = 1.0f;
                    _offButton.hidden = false;
                    _offButton.selected = false;
                ...
                case PGCameraFlashModeOn:
                {
                    ...
                    _onButton.frame = [self _selectedButtonFrameForSize:_onButton.frame.size
                                                   interfaceOrientation:_interfaceOrientation];
                    _onButton.alpha = 1.0f;
                    _onButton.hidden = false;
                    _onButton.selected = true;
                ...
                case PGCameraFlashModeAuto:
                {
                    ...
                    _autoButton.frame = [self _selectedButtonFrameForSize:_autoButton.frame.size
                                                     interfaceOrientation:_interfaceOrientation];
                    _autoButton.alpha = 1.0f;
                    _autoButton.hidden = false;
                    _autoButton.selected = false;
...
- (CGRect)_flashIconFrameForActive:(bool)active interfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    switch (interfaceOrientation)
    {
        ...
        {
            if (active)
                origin = CGPointMake(size.width - _flashIconView.frame.size.width - 5,
                                     (size.height - _flashIconView.frame.size.height) / 2);
            else
                origin = CGPointMake(size.width - _flashIconView.frame.size.width - 5,
                                     (size.height - _flashIconView.frame.size.height) / 2 - 9);
        }
            break;
        ...
        case UIInterfaceOrientationPortraitUpsideDown:
        {
            ...
            {
                CGFloat maxWidth = MAX(MAX(_offButton.frame.size.width,
                                           _onButton.frame.size.width),
                                       _autoButton.frame.size.width);
    ...
    return CGRectMake(origin.x,
                      origin.y,
                      _flashIconView.frame.size.width,
...
- (CGRect)_selectedButtonFrameForSize:(CGSize)buttonSize interfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    switch (interfaceOrientation)
    {
        ...
        case UIInterfaceOrientationPortraitUpsideDown:
        {
            ...
            origin = CGPointMake(iconFrame.origin.x + iconFrame.size.width - 3,
...
- (CGRect)_offButtonFrameForInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation
{
    ...
    return CGRectMake(size.width / 4 * 3 - _offButton.frame.size.width / 2,
                      ...
                      _offButton.frame.size.width,
                      _offButton.frame.size.height);
}
