...
static UIScrollView *TGFindScrollViewInView(UIView *view)
{
    ...
    {
        UIScrollView *scrollView = TGFindScrollViewInView(subview);
        if (scrollView != nil)
            return scrollView;
    }
...
UIScrollView *TGUIWebViewScrollView(UIWebView *webView)
{
    if (webView == nil)
        return nil;

    return TGFindScrollViewInView(webView);
}

bool TGShareItemsViaLegacyPasteboard(NSArray *items)
{
    ...
    for (id item in items)
    {
        if ([item isKindOfClass:[NSString class]])
            ...
            [strings addObject:[item absoluteString]];
        else if (image == nil && [item isKindOfClass:[UIImage class]])
            image = item;
    }

    UIPasteboard *pasteboard = [UIPasteboard generalPasteboard];
    if (strings.count != 0)
        pasteboard.string = [strings componentsJoinedByString:@"\n"];
    else if (image != nil)
        pasteboard.image = image;

    NSString *message = TGLocalized(@"Username.LinkCopied");
    if (message.length == 0 || [message isEqualToString:@"Username.LinkCopied"])
        message = @"Copied to Clipboard";
    [[[UIAlertView alloc] initWithTitle:nil message:message delegate:nil cancelButtonTitle:TGLocalized(@"Common.OK") otherButtonTitles:nil] show];
    return true;
}

id TGWeakValueMapTable(void)
{
    if (iosMajorVersion() < 6)
        return [[NSMutableDictionary alloc] init];
    
    Class mapTableClass = NSClassFromString(@"NSMapTable");
    if (mapTableClass != Nil)
    {
...
id TGWeakObjectHashTable(void)
{
    if (iosMajorVersion() < 6)
        return [[NSMutableSet alloc] init];
    
    Class hashTableClass = NSClassFromString(@"NSHashTable");
    if (hashTableClass != Nil)
...
static id TGIOS5ArrayObjectAtIndexedSubscript(id self, SEL __unused _cmd, NSUInteger index)
{
    return [self objectAtIndex:index];
}

static void TGIOS5MutableArraySetObjectAtIndexedSubscript(id self, SEL __unused _cmd, id object, NSUInteger index)
{
...
static id TGIOS5DictionaryObjectForKeyedSubscript(id self, SEL __unused _cmd, id key)
{
    return [self objectForKey:key];
}

static void TGIOS5MutableDictionarySetObjectForKeyedSubscript(id self, SEL __unused _cmd, id object, id key)
{
    if (object != nil)
...
static void TGIOS5InstallMethod(Class cls, SEL selector, IMP implementation, const char *types)
{
    if (cls == Nil)
        return;
...
static void TGInstallIOS5SubscriptingCompatibility(void)
{
    Method arraySourceMethod = class_getInstanceMethod([NSArray class], @selector(objectAtIndex:));
    Method dictionarySourceMethod = class_getInstanceMethod([NSDictionary class], @selector(objectForKey:));
    Method dictionarySetterSourceMethod = class_getInstanceMethod([NSMutableDictionary class], @selector(setObject:forKey:));
    
    const char *arrayGetterTypes =
    arraySourceMethod != NULL
    ? method_getTypeEncoding(arraySourceMethod)
    : "@@:I";
    
    const char *dictionaryGetterTypes =
    dictionarySourceMethod != NULL
    ? method_getTypeEncoding(dictionarySourceMethod)
    : "@@:@";
    
    const char *dictionarySetterTypes =
    dictionarySetterSourceMethod != NULL
    ? method_getTypeEncoding(dictionarySetterSourceMethod)
    : "v@:@@";
    
    TGIOS5InstallMethod(
                        [NSArray class],
                        @selector(objectAtIndexedSubscript:),
                        (IMP)TGIOS5ArrayObjectAtIndexedSubscript,
                        arrayGetterTypes
                        );
    ...
    TGIOS5InstallMethod(
                        ...
                        (IMP)TGIOS5MutableArraySetObjectAtIndexedSubscript,
                        "v@:@I"
                        );
    ...
    TGIOS5InstallMethod(
                        ...
                        (IMP)TGIOS5DictionaryObjectForKeyedSubscript,
                        dictionaryGetterTypes
                        );
    ...
    TGIOS5InstallMethod(
                        objc_getClass("__NSSingleEntryDictionaryI"),
                        @selector(objectForKeyedSubscript:),
                        (IMP)TGIOS5DictionaryObjectForKeyedSubscript,
                        dictionaryGetterTypes
                        );
    ...
    TGIOS5InstallMethod(
                        ...
                        (IMP)TGIOS5MutableDictionarySetObjectForKeyedSubscript,
                        dictionarySetterTypes
                        );
...
void SwizzleClassMethod(Class c, SEL orig, SEL new)
{
    ...
    {
        NSLog(@"Attempt to swizzle nonexistent class methods!");
        return;
    }
...
@interface UIImage (TGIOS4ResizableImageCompatibility)
- (UIImage *)tg_ios4_resizableImageWithCapInsets:(UIEdgeInsets)capInsets;
@end

@implementation UIImage (TGIOS4ResizableImageCompatibility)
...
- (UIImage *)tg_ios4_resizableImageWithCapInsets:(UIEdgeInsets)capInsets
{
    ...
    CGFloat left = MAX(0.0f, MIN(capInsets.left, sourceSize.width));
    CGFloat right = MAX(0.0f, MIN(capInsets.right, sourceSize.width - left));
    CGFloat top = MAX(0.0f, MIN(capInsets.top, sourceSize.height));
    CGFloat bottom = MAX(0.0f, MIN(capInsets.bottom, sourceSize.height - top));

    CGFloat centerWidth = sourceSize.width - left - right;
    CGFloat centerHeight = sourceSize.height - top - bottom;
    if (centerWidth <= 0.0f || centerHeight <= 0.0f)
        return [self stretchableImageWithLeftCapWidth:(NSInteger)CGFloor(left) topCapHeight:(NSInteger)CGFloor(top)];

    CGFloat scale = self.scale;
    if (scale <= 0.0f)
        scale = 1.0f;

    if (centerWidth <= 1.0f && centerHeight <= 1.0f)
        return [self stretchableImageWithLeftCapWidth:(NSInteger)CGFloor(left) topCapHeight:(NSInteger)CGFloor(top)];

    CGSize templateSize = CGSizeMake(left + right + 1.0f, top + bottom + 1.0f);
    CGFloat sourceX[4] = {0.0f, left, sourceSize.width - right, sourceSize.width};
    CGFloat sourceY[4] = {0.0f, top, sourceSize.height - bottom, sourceSize.height};
    CGFloat targetX[4] = {0.0f, left, left + 1.0f, templateSize.width};
    CGFloat targetY[4] = {0.0f, top, top + 1.0f, templateSize.height};

    UIGraphicsBeginImageContextWithOptions(templateSize, false, scale);
    for (int y = 0; y < 3; y++)
    {
        for (int x = 0; x < 3; x++)
        {
            CGRect sourceRect = CGRectMake(sourceX[x], sourceY[y], sourceX[x + 1] - sourceX[x], sourceY[y + 1] - sourceY[y]);
            CGRect targetRect = CGRectMake(targetX[x], targetY[y], targetX[x + 1] - targetX[x], targetY[y + 1] - targetY[y]);
            if (sourceRect.size.width <= 0.0f || sourceRect.size.height <= 0.0f || targetRect.size.width <= 0.0f || targetRect.size.height <= 0.0f)
                continue;

            CGFloat pixelMinX = CGFloor(CGRectGetMinX(sourceRect) * scale);
            CGFloat pixelMinY = CGFloor(CGRectGetMinY(sourceRect) * scale);
            CGFloat pixelMaxX = CGCeil(CGRectGetMaxX(sourceRect) * scale);
            CGFloat pixelMaxY = CGCeil(CGRectGetMaxY(sourceRect) * scale);
            CGRect pixelRect = CGRectMake(pixelMinX, pixelMinY, MAX(1.0f, pixelMaxX - pixelMinX), MAX(1.0f, pixelMaxY - pixelMinY));
            CGImageRef sliceRef = CGImageCreateWithImageInRect(self.CGImage, pixelRect);
            if (sliceRef != NULL)
            {
                UIImage *slice = [UIImage imageWithCGImage:sliceRef scale:scale orientation:UIImageOrientationUp];
                [slice drawInRect:targetRect blendMode:kCGBlendModeCopy alpha:1.0f];
    ...
    return [templateImage stretchableImageWithLeftCapWidth:(NSInteger)CGFloor(left) topCapHeight:(NSInteger)CGFloor(top)];
}
...
+ (void)load
{
    @autoreleasepool
    {
        TGInstallIOS5SubscriptingCompatibility();
    
        TGInstallCompatibilityInstanceMethod([UIImage class], @selector(resizableImageWithCapInsets:), @selector(tg_ios4_resizableImageWithCapInsets:));
        ...
        TGInstallCompatibilityInstanceMethod([UILabel class], @selector(attributedText), @selector(tg_ios5_attributedText));
        TGInstallCompatibilityInstanceMethod([UILabel class], @selector(setAttributedText:), @selector(tg_ios5_setAttributedText:));
        TGInstallCompatibilityInstanceMethod([UILabel class], @selector(minimumScaleFactor), @selector(tg_ios5_minimumScaleFactor));
        TGInstallCompatibilityInstanceMethod([UILabel class], @selector(setMinimumScaleFactor:), @selector(tg_ios5_setMinimumScaleFactor:));
        TGInstallCompatibilityInstanceMethod([UITextField class], @selector(attributedPlaceholder), @selector(tg_ios5_attributedPlaceholder));
        TGInstallCompatibilityInstanceMethod([UITextField class], @selector(setAttributedPlaceholder:), @selector(tg_ios5_setAttributedPlaceholder:));
    
        TGInstallCompatibilityInstanceMethod([PSUICollectionView class], @selector(startInteractiveTransitionToCollectionViewLayout:completion:), @selector(tg_ios6_startInteractiveTransitionToCollectionViewLayout:completion:));
        TGInstallCompatibilityInstanceMethod([PSUICollectionView class], @selector(finishInteractiveTransition), @selector(tg_ios6_finishInteractiveTransition));
        ...
        TGInstallCompatibilityInstanceMethod([UITableView class], @selector(separatorInset), @selector(tg_ios6_separatorInset));
        TGInstallCompatibilityInstanceMethod([UITableView class], @selector(setSeparatorInset:), @selector(tg_ios6_setSeparatorInset:));
        TGInstallCompatibilityInstanceMethod([UITableViewCell class], @selector(separatorInset), @selector(tg_ios6_separatorInset));
        TGInstallCompatibilityInstanceMethod([UITableViewCell class], @selector(setSeparatorInset:), @selector(tg_ios6_setSeparatorInset:));
        ...
        TGInstallCompatibilityInstanceMethod([UIView class], @selector(tintColor), @selector(tg_ios6_tintColor));
        TGInstallCompatibilityInstanceMethod([UIView class], @selector(setTintColor:), @selector(tg_ios6_setTintColor:));
        TGInstallCompatibilityInstanceMethod([UIView class], @selector(layoutMargins), @selector(tg_ios6_layoutMargins));
        TGInstallCompatibilityInstanceMethod([UIView class], @selector(setLayoutMargins:), @selector(tg_ios6_setLayoutMargins:));
        TGInstallCompatibilityInstanceMethod([UINavigationBar class], @selector(barTintColor), @selector(tg_ios6_barTintColor));
        TGInstallCompatibilityInstanceMethod([UINavigationBar class], @selector(setBarTintColor:), @selector(tg_ios6_setBarTintColor:));
        ...
        TGInstallCompatibilityInstanceMethod([UIViewController class], @selector(automaticallyAdjustsScrollViewInsets), @selector(tg_ios6_automaticallyAdjustsScrollViewInsets));
        TGInstallCompatibilityInstanceMethod([UIViewController class], @selector(setAutomaticallyAdjustsScrollViewInsets:), @selector(tg_ios6_setAutomaticallyAdjustsScrollViewInsets:));
        TGInstallCompatibilityInstanceMethod([UIViewController class], @selector(preferredContentSize), @selector(tg_ios6_preferredContentSize));
        TGInstallCompatibilityInstanceMethod([UIViewController class], @selector(setPreferredContentSize:), @selector(tg_ios6_setPreferredContentSize:));
        TGInstallCompatibilityInstanceMethod([UIViewController class], @selector(traitCollection), @selector(tg_ios6_traitCollection));
        TGInstallCompatibilityInstanceMethod([UIViewController class], @selector(transitionCoordinator), @selector(tg_ios6_transitionCoordinator));
        ...
        TGInstallCompatibilityInstanceMethod([UITextView class], @selector(attributedText), @selector(tg_ios5_attributedText));
        TGInstallCompatibilityInstanceMethod([UITextView class], @selector(setAttributedText:), @selector(tg_ios5_setAttributedText:));
        TGInstallCompatibilityInstanceMethod([UITextView class], @selector(typingAttributes), @selector(tg_ios5_typingAttributes));
        TGInstallCompatibilityInstanceMethod([UITextView class], @selector(setTypingAttributes:), @selector(tg_ios5_setTypingAttributes:));
...
@implementation UILabel (LegacyComponentsAttributedTextCompatibility)

- (NSAttributedString *)tg_ios5_attributedText
{
    Class attributedStringClass = NSClassFromString(@"NSAttributedString");
    if (attributedStringClass == Nil)
        return nil;

    return [[attributedStringClass alloc] initWithString:self.text == nil ? @"" : self.text];
}

- (void)tg_ios5_setAttributedText:(NSAttributedString *)attributedText
{
    self.text = attributedText.string;
}

- (CGFloat)tg_ios5_minimumScaleFactor
{
    if (self.font.pointSize <= FLT_EPSILON)
        return 0.0f;

    return self.minimumFontSize / self.font.pointSize;
}

- (void)tg_ios5_setMinimumScaleFactor:(CGFloat)minimumScaleFactor
{
    self.minimumFontSize = self.font.pointSize * minimumScaleFactor;
}

@end

@implementation UITextField (LegacyComponentsAttributedPlaceholderCompatibility)

- (NSAttributedString *)tg_ios5_attributedPlaceholder
{
    return nil;
}

- (void)tg_ios5_setAttributedPlaceholder:(NSAttributedString *)attributedPlaceholder
{
...
- (UICollectionViewTransitionLayout *)tg_ios6_startInteractiveTransitionToCollectionViewLayout:(PSUICollectionViewLayout *)layout completion:(UICollectionViewLayoutInteractiveTransitionCompletion)completion
{
...
@implementation UITableView (LegacyComponentsSeparatorInsetCompatibility)

- (UIEdgeInsets)tg_ios6_separatorInset
{
    return UIEdgeInsetsZero;
}

- (void)tg_ios6_setSeparatorInset:(UIEdgeInsets)__unused separatorInset
{
}
...
@implementation UITableViewCell (LegacyComponentsSeparatorInsetCompatibility)
...
- (UIColor *)tg_ios6_tintColor
{
    return nil;
}

- (void)tg_ios6_setTintColor:(UIColor *)__unused tintColor
{
}

- (UIEdgeInsets)tg_ios6_layoutMargins
{
    return UIEdgeInsetsZero;
}

- (void)tg_ios6_setLayoutMargins:(UIEdgeInsets)__unused layoutMargins
{
}

@end

@implementation UINavigationBar (LegacyComponentsBarTintCompatibility)

- (UIColor *)tg_ios6_barTintColor
{
    return self.tintColor;
}

- (void)tg_ios6_setBarTintColor:(UIColor *)barTintColor
{
...
- (BOOL)tg_ios6_automaticallyAdjustsScrollViewInsets
{
    return false;
}

- (void)tg_ios6_setAutomaticallyAdjustsScrollViewInsets:(BOOL)__unused automaticallyAdjustsScrollViewInsets
{
}

- (CGSize)tg_ios6_preferredContentSize
{
    return CGSizeZero;
}

- (void)tg_ios6_setPreferredContentSize:(CGSize)__unused preferredContentSize
{
}

- (UITraitCollection *)tg_ios6_traitCollection
{
    return nil;
}

- (id<UIViewControllerTransitionCoordinator>)tg_ios6_transitionCoordinator
{
    return nil;
}
...
- (NSDictionary *)tg_ios5_typingAttributes
{
    return nil;
}

- (void)tg_ios5_setTypingAttributes:(NSDictionary *)__unused typingAttributes
{
}
...
static Class TGIOS6WebKitRuntimeClass(NSString *name, Class compatibilityClass)
{
    ...
    if (systemClass == Nil)
    {
        ...
        systemClass = NSClassFromString(name);
    }
    if (systemClass == Nil || systemClass == compatibilityClass)
        return Nil;
    return systemClass;
}

#define TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS(systemName) \
+ (id)allocWithZone:(NSZone *)zone \
{ \
    Class systemClass = TGIOS6WebKitRuntimeClass(@systemName, self); \
    if (systemClass != Nil) \
        return [systemClass allocWithZone:zone]; \
    return [super allocWithZone:zone]; \
} \
+ (id)forwardingTargetForSelector:(SEL)selector \
{ \
    Class systemClass = TGIOS6WebKitRuntimeClass(@systemName, self); \
    if (systemClass != Nil && [systemClass respondsToSelector:selector]) \
        return systemClass; \
    return nil; \
}

@implementation WKFrameInfo
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKFrameInfo")
@end

@implementation WKNavigation
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKNavigation")
@end

@implementation WKNavigationAction
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKNavigationAction")
...
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKUserScript")
...
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKPreferences")
@end

@implementation WKUserContentController
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKUserContentController")
...
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKWebViewConfiguration")
@end

@implementation WKWebView
TGIOS6_WEBKIT_RUNTIME_PROXY_METHODS("WKWebView")
