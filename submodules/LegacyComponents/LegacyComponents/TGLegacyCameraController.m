                ...
                if (referenceUrl != nil)
                {
                    ...
                    [TGImagePickerController loadAssetWithUrl:referenceUrl completion:^(ALAsset *asset)
                    {
                        if (asset != nil)
                        {
                            ...
                            {
                                [delegate legacyCameraControllerCompletedWithDocument:[NSURL fileURLWithPath:tempFileName] fileName:TGMediaAssetLegacyRepresentationFileName(representation) mimeType:mimeType];
                            });
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        [exportSession exportAsynchronouslyWithCompletionHandler:^
                        {
                            ...
                            if (endProcessing)
                            {
                                ...
                                if (success)
                                {
                                    AVAsset *mp4Asset = [[AVURLAsset alloc] initWithURL:[NSURL fileURLWithPath:videoMp4FilePath] options:nil];
