...
+ (SSignal *)imageForAsset:(TGMediaAsset *)asset imageType:(TGMediaAssetImageType)imageType size:(CGSize)size allowNetworkAccess:(bool)__unused allowNetworkAccess
{
    ...
    switch (imageType)
    {
        ...
        {
            return [SSignal single:TGMediaAssetLegacyAspectRatioThumbnailImage(asset.backingLegacyAsset)];
        }
            break;
...
+ (SSignal *)fileAttributesForAsset:(TGMediaAsset *)asset
{
    return [[[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
    {
        ...
        attributes.dimensions = TGMediaAssetLegacyRepresentationDimensions(representation);
...
+ (SSignal *)_editedImageWithCGImage:(CGImageRef)cgImage representation:(ALAssetRepresentation *)representation
{
    return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
    {
        if (NSClassFromString(@"CIContext") == Nil || NSClassFromString(@"CIImage") == Nil)
        {
        ...
        if (iosMajorVersion() >= 6 &&
            [CIFilter respondsToSelector:@selector(filterArrayFromSerializedXMP:inputImageExtent:error:)])
        {
