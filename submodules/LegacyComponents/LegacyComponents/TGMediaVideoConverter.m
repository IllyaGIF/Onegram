    ...
    return [[SSignal alloc] initWithGenerator:^id<SDisposable>(SSubscriber *subscriber)
    {
        ...
        [avAsset loadValuesAsynchronouslyForKeys:requiredKeys completionHandler:^
        {
            [queue dispatch:^
            {
                ...
                TGDispatchAfter(1.0, (dispatch_queue_t)[queue _dispatch_queueRaw], ^
                {
                    if (watcher != nil)
...
+ (void)processWithConversionContext:(SAtomic *)context_ completionBlock:(void (^)(void))completionBlock
{
    ...
    dispatch_group_notify(dispatchGroup, (dispatch_queue_t)[context.queue _dispatch_queueRaw], ^
    {
        TGMediaVideoConversionContext *context = [context_ value];
        ...
        else
        {
            ...
            if (!audioProcessingFailed && !videoProcessingFailed && context.assetReader.status != AVAssetReaderStatusFailed)
            {
                ...
                if ([context.assetWriter respondsToSelector:@selector(finishWritingWithCompletionHandler:)])
                    [context.assetWriter finishWritingWithCompletionHandler:finishCompletion];
                else
                {
                    [context.assetWriter finishWriting];
                    finishCompletion();
                }
            }
            else
...
- (void)startWithTimeRange:(CMTimeRange)timeRange progressBlock:(void (^)(CGFloat progress))progressBlock completionBlock:(void (^)(void))completionBlock
{
    ...
    [_assetWriterInput requestMediaDataWhenReadyOnQueue:(dispatch_queue_t)[_queue _dispatch_queueRaw] usingBlock:^
    {
        if (_finished)
            return;
...
- (dispatch_source_t)_setup
{
    ...
    if (fd > 0)
    {
        dispatch_source_t source = dispatch_source_create(DISPATCH_SOURCE_TYPE_TIMER, 0, 0, (dispatch_queue_t)[_queue _dispatch_queueRaw]);
