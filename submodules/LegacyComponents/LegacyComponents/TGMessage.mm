...
static NSMutableDictionary *mediaAttachmentParsers = nil;
...
{
    if (mediaAttachmentParsers == nil)
        mediaAttachmentParsers = [[NSMutableDictionary alloc] init];
    if (parser != nil)
        [mediaAttachmentParsers setObject:parser forKey:[NSNumber numberWithInt:type]];
}
...
+ (NSArray *)parseMediaAttachments:(NSData *)data
{
    ...
    {
        int32_t count = 0;
        if ([is read:(uint8_t *)&count maxLength:4] != 4 || count < 0 || (NSUInteger)count > data.length / 4)
            valid = false;

        if (valid)
            attachments = [[NSMutableArray alloc] initWithCapacity:(NSUInteger)count];

        for (int32_t i = 0; valid && i < count; i++)
        {
            int32_t type = 0;
            if ([is read:(uint8_t *)&type maxLength:4] != 4)
            {
                valid = false;
                break;
            }

            id<TGMediaAttachmentParser> parser = [mediaAttachmentParsers objectForKey:[NSNumber numberWithInt:type]];
            if (parser == nil)
            {
            ...
            TGMediaAttachment *attachment = [parser parseMediaAttachment:is];
            if (attachment != nil)
    ...
    [is close];
    return valid ? [NSArray arrayWithArray:attachments] : [NSArray array];
}
...
- (NSData *)serializeContentProperties
{
    ...
    [_contentProperties enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, __unused BOOL *stop)
    {
        if ([value respondsToSelector:@selector(encodeWithKeyValueCoder:)])
            [encoder encodeObject:(id<PSCoding>)value forKey:key];
        else
            TGLegacyLog(@"STABILITY contentProperty.skip key=%@ class=%@", key, NSStringFromClass([value class]));
    }];
...
+ (NSData *)serializeContentProperties:(NSDictionary *)contentProperties
{
    ...
    PSKeyValueEncoder *encoder = [[PSKeyValueEncoder alloc] init];
    [contentProperties enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, __unused BOOL *stop)
    {
        if ([value respondsToSelector:@selector(encodeWithKeyValueCoder:)])
            [encoder encodeObject:(id<PSCoding>)value forKey:key];
        else
