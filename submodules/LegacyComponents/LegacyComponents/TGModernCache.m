...
- (void)removeValueForKey:(NSData *)key
{
    ...
    [_queue dispatch:^
    {
        ...
        [_keyValueStore readWriteInTransaction:^(id<PSKeyValueReader,PSKeyValueWriter> readerWriter)
        {
            ...
            int8_t keyspace = TGModernCacheKeyspaceLastUsageByPath;
            [primaryKeyData appendBytes:&keyspace length:1];
            [primaryKeyData appendData:key];
            PSData primaryKey = {.data = (void *)primaryKeyData.bytes, .length = primaryKeyData.length};

            PSData value;
            bool hadMetadata = [readerWriter readValueForRawKey:&primaryKey value:&value];
            if (hadMetadata)
            {
                int32_t sortingValue = 0;
                if (value.length >= 4)
                ...
                [readerWriter deleteValueForRawKey:&primaryKey];

                NSMutableData *indexKeyData = [[NSMutableData alloc] init];
                keyspace = TGModernCacheKeyspacePathAndSizeByLastUsage;
                [indexKeyData appendBytes:&keyspace length:1];
                [indexKeyData appendBytes:&sortingValue length:4];
                [indexKeyData appendData:key];
                PSData indexKey = {.data = (void *)indexKeyData.bytes, .length = indexKeyData.length};
                [readerWriter deleteValueForRawKey:&indexKey];

                NSUInteger currentSize = [self _getCurrentSize:readerWriter];
                [self setCurrentSize:readerWriter size:currentSize > fileSize ? currentSize - fileSize : 0];
            }
        }];
