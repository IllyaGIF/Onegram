...
@interface TGPhotoEditorCollectionView () <PSUICollectionViewDelegateFlowLayout, UIGestureRecognizerDelegate>
{
    NSIndexPath *_selectedItemIndexPath;
    PSUICollectionViewFlowLayout *_layout;
    
    bool _landscape;
    CGFloat _nameWidth;
}

@property (nonatomic, weak) id<PSUICollectionViewDelegate> realDelegate;
...
- (bool)hasAnyTracking
{
    ...
    for (PSUICollectionViewCell *cell in self.visibleCells)
    {
...
- (void)setDelegate:(id<PSUICollectionViewDelegate>)delegate
{
    if (delegate == nil)
    {
...
{
    [(PSUICollectionViewFlowLayout *)self.collectionViewLayout setMinimumLineSpacing:minimumLineSpacing];
    [self.collectionViewLayout invalidateLayout];
}

- (void)setMinimumInteritemSpacing:(CGFloat)minimumInteritemSpacing
{
    [(PSUICollectionViewFlowLayout *)self.collectionViewLayout setMinimumInteritemSpacing:minimumInteritemSpacing];
    [self.collectionViewLayout invalidateLayout];
}
...
- (void)setSelectedItemIndexPath:(NSIndexPath *)indexPath
{
    ...
    {
        PSUICollectionViewCell *cell = [self cellForItemAtIndexPath:i];
...
- (PSUICollectionViewCell *)collectionView:(PSUICollectionView *)__unused collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    id<TGPhotoEditorCollectionViewFiltersDataSource> filtersDataSource = self.filtersDataSource;
    id<TGPhotoEditorCollectionViewToolsDataSource> toolsDataSource = self.toolsDataSource;
...
- (void)setCellsHidden:(bool)hidden excludeCell:(PSUICollectionViewCell *)excludeCell animated:(bool)animated
{
    void (^block)(void) = ^
    {
        for (PSUICollectionViewCell *cell in self.visibleCells)
...
- (void)collectionView:(PSUICollectionView *)__unused collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([filtersDataSource respondsToSelector:@selector(collectionView:didSelectFilterWithIndex:)])
    {
        ...
        if (!vertical)
        {
            ...
            itemSize = ((PSUICollectionViewFlowLayout *)self.collectionViewLayout).itemSize.width;
            startInset = self.contentInset.left;
            endInset = self.contentInset.right;
            triggerOffset = TGPhotoEditorEdgeScrollTriggerOffset.x;
            targetOverlap = itemSize / 2 + ((PSUICollectionViewFlowLayout *)self.collectionViewLayout).minimumLineSpacing;
        }
        else
        {
            ...
            itemSize = ((PSUICollectionViewFlowLayout *)self.collectionViewLayout).itemSize.height;
...
- (BOOL)collectionView:(PSUICollectionView *)__unused collectionView shouldSelectItemAtIndexPath:(NSIndexPath *)__unused indexPath
{
    return false;
}
...
{
    id<PSUICollectionViewDelegate> realDelegate = self.realDelegate;
