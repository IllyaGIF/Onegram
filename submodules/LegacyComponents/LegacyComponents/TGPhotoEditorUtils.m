...
{
    if (image == nil || image.CGImage == NULL || targetSize.width < 1.0f || targetSize.height < 1.0f)
        return NULL;

    if (TGOrientationIsSideward(image.imageOrientation, NULL))
        targetSize = CGSizeMake(targetSize.height, targetSize.width);

    size_t width = (size_t)MAX(1.0f, CGFloor(targetSize.width));
    size_t height = (size_t)MAX(1.0f, CGFloor(targetSize.height));
    CGColorSpaceRef colorSpace = CGColorSpaceCreateDeviceRGB();
    if (colorSpace == NULL)
        return NULL;

    CGContextRef context = CGBitmapContextCreate(NULL, width, height, 8, width * 4, colorSpace, kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrder32Host);
    ...
    CGContextDrawImage(context, CGRectMake(0.0f, 0.0f, (CGFloat)width, (CGFloat)height), image.CGImage);
