...
void TGPresentViewController(UIViewController *presentingController, UIViewController *viewController, BOOL animated, void (^completion)(void))
{
    if (presentingController == nil || viewController == nil)
    {
        if (completion != nil)
            completion();
        return;
    ...
    {
        [presentingController presentViewController:viewController animated:animated completion:completion];
        return;
    }
    ...
    if (completion != nil)
    {
        ...
        [presentingController presentModalViewController:viewController animated:animated];
...
static UIViewController *TGModalContainerController(UIViewController *controller)
{
...
static UIViewController *TGModalPresentingController(UIViewController *controller)
{
    UIViewController *targetController = TGModalContainerController(controller);
...
void TGDismissViewController(UIViewController *controller, BOOL animated, void (^completion)(void))
{
    if (controller == nil)
    {
        if (completion != nil)
    ...
    UIViewController *dismissingController = controller;
    if (controller.modalViewController == nil)
    {
        UIViewController *presentingController = TGModalPresentingController(controller);
        if (presentingController != nil)
            dismissingController = presentingController;
    }

    if (iosMajorVersion() >= 5)
    {
        [dismissingController dismissViewControllerAnimated:animated completion:completion];
        return;
    }
    ...
    if (completion != nil)
    {
        [CATransaction begin];
        [CATransaction setCompletionBlock:completion];
        [dismissingController dismissModalViewControllerAnimated:animated];
        [CATransaction commit];
    }
    else
    {
...
@interface TGIOS4BarButtonItemsToolbar : UIToolbar
@end

@implementation TGIOS4BarButtonItemsToolbar
...
static CGFloat TGIOS4BarButtonItemWidth(UIBarButtonItem *item)
{
    if (item.customView != nil)
    {
        CGSize size = item.customView.bounds.size;
        if (size.width < 1.0f)
            size = [item.customView sizeThatFits:CGSizeMake(160.0f, 44.0f)];
        if (size.width > 0.0f)
            return size.width;
    }
    ...
    {
        CGSize size = [item.title sizeWithFont:[UIFont boldSystemFontOfSize:12.0f]];
        return MAX(30.0f, size.width + 20.0f);
    }

    if (item.image != nil)
        return MAX(30.0f, item.image.size.width + 20.0f);

    return 32.0f;
}

static UIBarButtonItem *TGIOS4CombinedRightBarButtonItem(NSArray *items)
{
    ...
    NSMutableArray *toolbarItems = [[NSMutableArray alloc] initWithCapacity:count];
    CGFloat width = 0.0f;
    for (NSInteger index = (NSInteger)count - 1; index >= 0; index--)
    {
        UIBarButtonItem *item = [items objectAtIndex:(NSUInteger)index];
        [toolbarItems addObject:item];
        width += TGIOS4BarButtonItemWidth(item);
    }
    width += (count - 1) * 4.0f;

    TGIOS4BarButtonItemsToolbar *toolbar = [[TGIOS4BarButtonItemsToolbar alloc] initWithFrame:CGRectMake(0.0f, 0.0f, width, 44.0f)];
...
static UIInterfaceOrientation TGInterfaceOrientationForDeviceOrientation(UIDeviceOrientation deviceOrientation, UIInterfaceOrientation fallbackOrientation)
{
...
static UIViewController *TGAutorotationControllerForRootController(UIViewController *controller)
{
    UIViewController *currentController = controller;
...
static CGAffineTransform TGTransformForInterfaceOrientation(UIInterfaceOrientation interfaceOrientation)
{
...
static void TGSetLegacyInterfaceOrientation(UIInterfaceOrientation interfaceOrientation, BOOL animated)
{
    UIApplication *application = [UIApplication sharedApplication];
    UIInterfaceOrientation previousOrientation = application.statusBarOrientation;
    if (previousOrientation == interfaceOrientation)
        return;

    NSTimeInterval duration = animated ? 0.3 : 0.0;
    if (animated && [application respondsToSelector:@selector(statusBarOrientationAnimationDuration)])
        duration = application.statusBarOrientationAnimationDuration;
    ...
    for (UIWindow *window in application.windows)
    {
        ...
        UIViewController *controller = TGAutorotationControllerForRootController(rootController);
        if (controller == nil || ![controller shouldAutorotateToInterfaceOrientation:interfaceOrientation])
            continue;

        [windows addObject:window];
        [controllers addObject:controller];
        [controller willRotateToInterfaceOrientation:interfaceOrientation duration:duration];
    }

    if (windows.count == 0)
        return;

    [application setStatusBarOrientation:interfaceOrientation animated:animated];
    ...
    void (^animations)(void) = ^
    {
        for (NSUInteger i = 0; i < windows.count; i++)
        {
            ...
            CGFloat shortSide = MIN(windowSize.width, windowSize.height);
            CGFloat longSide = MAX(windowSize.width, windowSize.height);
            CGSize targetSize = UIInterfaceOrientationIsLandscape(interfaceOrientation) ? CGSizeMake(longSide, shortSide) : CGSizeMake(shortSide, longSide);

            rootView.transform = TGTransformForInterfaceOrientation(interfaceOrientation);
            rootView.bounds = CGRectMake(0.0f, 0.0f, targetSize.width, targetSize.height);
            rootView.center = CGPointMake(CGRectGetMidX(window.bounds), CGRectGetMidY(window.bounds));
    ...
    void (^completion)(BOOL) = ^(BOOL finished)
    {
        ...
        for (UIViewController *controller in controllers)
            [controller didRotateFromInterfaceOrientation:previousOrientation];
    };
...
+ (void)setInterfaceOrientation:(UIInterfaceOrientation)interfaceOrientation animated:(bool)animated
{
    ...
    TGSetLegacyInterfaceOrientation(interfaceOrientation, animated);
}
...
+ (void)attemptAutorotation
{
    ...
    UIInterfaceOrientation targetOrientation = TGInterfaceOrientationForDeviceOrientation(device.orientation, [UIApplication sharedApplication].statusBarOrientation);
    [device endGeneratingDeviceOrientationNotifications];

    TGSetLegacyInterfaceOrientation(targetOrientation, true);
}
...
- (void)_commonViewControllerInit:(id<LegacyComponentsContext>)context
{
    ...
    if (iosMajorVersion() >= 5)
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(viewControllerKeyboardWillChangeFrame:) name:UIKeyboardWillShowNotification object:nil];
...
- (BOOL)shouldAutorotate
{
    ...
    if (controller != nil)
    {
        if ([controller respondsToSelector:@selector(shouldAutorotate)])
        {
            if (![controller shouldAutorotate])
                return false;
        }
        else if (![controller shouldAutorotateToInterfaceOrientation:[UIApplication sharedApplication].statusBarOrientation])
        {
            return false;
        }
    }
...
- (void)_setRightBarButtonItems:(NSArray *)rightBarButtonItems animated:(BOOL)animated
{
    if (iosMajorVersion() < 5)
    {
        [[self _currentNavigationItem] setRightBarButtonItem:TGIOS4CombinedRightBarButtonItem(rightBarButtonItems) animated:animated];
        return;
    }

    if (rightBarButtonItems.count < 2)
    {
        UIBarButtonItem *rightBarButtonItem = rightBarButtonItems.count == 0 ? nil : [rightBarButtonItems objectAtIndex:0];
...
- (void)presentViewController:(UIViewController *)viewControllerToPresent animated:(BOOL)flag completion:(void (^)())completion
{
    ...
    if (iosMajorVersion() >= 8 && self.modalViewController != nil && [self.modalViewController isKindOfClass:[UIAlertController class]])
    {
        dispatch_async(dispatch_get_main_queue(), ^
        {
