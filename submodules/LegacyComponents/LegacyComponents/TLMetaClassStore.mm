...
- (id<TLObject>)TLdeserialize:(NSInputStream *)is signature:(int32_t)__unused signature environment:(id<TLSerializationEnvironment>)environment context:(TLSerializationContext *)__unused context error:(__autoreleasing NSError **)error
{
    ...
    TGLog(@"VIEWS parsed messageViews count=%d", (int)viewCounts.count);
...
- (id<TLObject>)TLdeserialize:(NSInputStream *)is signature:(int32_t)signature environment:(id<TLSerializationEnvironment>)__unused environment context:(TLSerializationContext *)__unused context error:(__autoreleasing NSError **)error
{
    if (signature == (int32_t)0xa5d72105)
    {
        ...
        {
            IOS6_NOOP_LOG(@"AUTH updateDialogFilterOrder expected Vector<int>, got 0x%08x", vectorMarker);
            if (error != NULL)
                *error = nil;
            return self;
        }
        ...
        IOS6_NOOP_LOG(@"AUTH skipped updateDialogFilterOrder count=%d", count);
        return self;
    }
    
    IOS6_NOOP_LOG(@"AUTH skipped updateDialogFilters sig=0x%08x", signature);
    return self;
}
...
void TLMetaClassStore::mergeScheme(TLScheme *scheme)
{
    ...
    {   
        CFAbsoluteTime mergePerfStart=CFAbsoluteTimeGetCurrent(), mergePerfLast=mergePerfStart;
#define MERGEPERF(label) do { CFAbsoluteTime n=CFAbsoluteTimeGetCurrent(); NSLog(@"MERGEPERF %@ total=%.1fms delta=%.1fms",label,(n-mergePerfStart)*1000.0,(n-mergePerfLast)*1000.0); mergePerfLast=n; } while(0)
        NSLog(@"MERGEPERF BEGIN");
        ...
        IOS6_NOOP_LOG(@"AUTH registered modern user constructor alias 0xd7c0ff9c");
        ...
        MERGEPERF(@"resolve constructors");
        ...
        MERGEPERF(@"manual snapshot");
        NSLog(@"MERGEPERF END total=%.1fms",(CFAbsoluteTimeGetCurrent()-mergePerfStart)*1000.0);
#undef MERGEPERF
    }
}
