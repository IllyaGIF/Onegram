...
static bool MTPBKDF2HMAC(CCHmacAlgorithm algorithm, size_t digestLength, const void *password, size_t passwordLength, const void *salt, size_t saltLength, uint32_t rounds, void *output, size_t outputLength)
{
    if (rounds == 0 || digestLength == 0 || digestLength > CC_SHA512_DIGEST_LENGTH || output == NULL)
        return false;

    uint8_t u[CC_SHA512_DIGEST_LENGTH];
    uint8_t next[CC_SHA512_DIGEST_LENGTH];
    uint8_t blockResult[CC_SHA512_DIGEST_LENGTH];
    uint8_t blockIndexBytes[4];
    ...
    {
        blockIndexBytes[0] = (uint8_t)(blockIndex >> 24);
        blockIndexBytes[1] = (uint8_t)(blockIndex >> 16);
        blockIndexBytes[2] = (uint8_t)(blockIndex >> 8);
        blockIndexBytes[3] = (uint8_t)blockIndex;

        CCHmacContext context;
        CCHmacInit(&context, algorithm, password, passwordLength);
        if (saltLength != 0)
            CCHmacUpdate(&context, salt, saltLength);
        CCHmacUpdate(&context, blockIndexBytes, sizeof(blockIndexBytes));
        CCHmacFinal(&context, u);
        memcpy(blockResult, u, digestLength);

        for (uint32_t round = 1; round < rounds; round++)
        {
            CCHmac(algorithm, password, passwordLength, u, digestLength, next);
            memcpy(u, next, digestLength);
            for (size_t i = 0; i < digestLength; i++)
                blockResult[i] ^= u[i];
        }

        size_t remaining = outputLength - outputOffset;
        size_t copyLength = remaining < digestLength ? remaining : digestLength;
        memcpy(outputBytes + outputOffset, blockResult, copyLength);
...
NSData * MTPBKDF2(NSData *data, NSData *salt, int rounds)
{
    ...
    const size_t hashLength = CC_SHA512_DIGEST_LENGTH;
    NSMutableData *result = [[NSMutableData alloc] initWithLength:hashLength];
    if (!MTPBKDF2HMAC(kCCHmacAlgSHA512, hashLength, data.bytes, data.length, salt.bytes, salt.length, (uint32_t)rounds, result.mutableBytes, hashLength))
        return nil;

    return result;
}

NSData * MTPBKDF2HMACSHA256(NSData *data, NSData *salt, int rounds, NSUInteger length)
{
    if (rounds < 1 || length == 0)
        return nil;

    NSMutableData *result = [[NSMutableData alloc] initWithLength:length];
    if (!MTPBKDF2HMAC(kCCHmacAlgSHA256, CC_SHA256_DIGEST_LENGTH, data.bytes, data.length, salt.bytes, salt.length, (uint32_t)rounds, result.mutableBytes, length))
        return nil;
