...
static void MTQueueIOS6PerfMaybeLog(const char *name, id queue, bool synchronous, CFAbsoluteTime elapsedMs)
{
    ...
    while (false) NSLog(@"PERF slowMTQueue %@ queue=%s ptr=%p sync=%d ms=%.1f skipped=%d", MTQueueIOS6PerfBuildTag(), name != NULL ? name : "unnamed", queue, synchronous ? 1 : 0, elapsedMs, skippedNow);
