...
void AudioMixer::AddInput(std::tr1::shared_ptr<MediaStreamItf> input){
...
void AudioMixer::RemoveInput(std::tr1::shared_ptr<MediaStreamItf> input){
	MutexGuard m(inputsMutex);
...
void AudioMixer::SetInputVolume(std::tr1::shared_ptr<MediaStreamItf> input, float volumeDB){
	MutexGuard m(inputsMutex);
	for(std::vector<MixerInput>::iterator i=inputs.begin();i!=inputs.end();++i){
		if(i->source==input){
