...
private:
	...
	public:
		...
		virtual void Start();
		virtual void Stop();
		void AddInput(std::tr1::shared_ptr<MediaStreamItf> input);
		void RemoveInput(std::tr1::shared_ptr<MediaStreamItf> input);
		void SetInputVolume(std::tr1::shared_ptr<MediaStreamItf> input, float volumeDB);
		void SetEchoCanceller(EchoCanceller* aec);
	private:
		void RunThread(void* arg);
		struct MixerInput{
			std::tr1::shared_ptr<MediaStreamItf> source;
			float multiplier;
		};
		Mutex inputsMutex;
