...
VoIPController::VoIPController() : activeNetItfName(""),
	...
	std::tr1::shared_ptr<Stream> stm=std::tr1::shared_ptr<Stream>(new Stream());
	stm->id=1;
...
VoIPController::~VoIPController(){
	...
	for(vector<std::tr1::shared_ptr<Stream>>::iterator _stm=incomingStreams.begin();_stm!=incomingStreams.end();++_stm){
		//LOGD("before delete jitter buffer");
		std::tr1::shared_ptr<Stream> stm=*_stm;
		/*if(stm->jitterBuffer){
...
#endif
	/*for(vector<std::tr1::shared_ptr<Stream>>::iterator stm=incomingStreams.begin();stm!=incomingStreams.end();++stm){
...
void VoIPController::SetRemoteEndpoints(vector<Endpoint> endpoints, bool allowP2p, int32_t connectionMaxLayer){
	...
	preferredRelay.reset();
	{
		...
		for(vector<Endpoint>::iterator itrtr=endpoints.begin();itrtr!=endpoints.end();++itrtr){
			this->endpoints.push_back(std::tr1::shared_ptr<Endpoint>(new Endpoint(*itrtr)));
...
void VoIPController::SendInit(){
	{
		...
		LOGI("VOIP init.begin seq=%u endpoints=%u layer=%d useMTProto2=%d outgoing=%d", initSeq, (unsigned int)endpoints.size(), connectionMaxLayer, (int)useMTProto2, (int)isOutgoing);
		for(std::tr1::shared_ptr<Endpoint>& e:endpoints){
			...
			LOGI("VOIP init.queue seq=%u endpoint=%lld type=%d addr=%s v6=%s port=%u len=%u", initSeq, (long long)e->id, (int)e->type, e->address.ToString().c_str(), e->v6address.ToString().c_str(), (unsigned int)e->port, (unsigned int)out.GetLength());
		}
	}
...
void VoIPController::RunRecvThread(void* arg){
	...
	while(runReceiver){
		...
		std::tr1::shared_ptr<Endpoint> srcEndpoint;
...
void VoIPController::RunSendThread(void* arg){
	...
	while(runReceiver){
		...
		if(pkt.data){
			std::tr1::shared_ptr<Endpoint> endpoint;
			if(pkt.endpoint){
...
void VoIPController::ProcessIncomingPacket(NetworkPacket &packet, std::tr1::shared_ptr<Endpoint> srcEndpoint){
	   ...
	   && *reinterpret_cast<uint64_t*>(buffer+16)==0xFFFFFFFFFFFFFFFFLL && *reinterpret_cast<uint32_t*>(buffer+24)==0xFFFFFFFF){
		...
		if(tlid==TLID_UDP_REFLECTOR_SELF_INFO){
			if(srcEndpoint->type==Endpoint::TYPE_UDP_RELAY /*&& udpConnectivityState==UDP_PING_SENT*/ && in.Remaining()>=32){
				...
				LOGI("VOIP recv.relaySelf endpoint=%lld addr=%s port=%u query=%lld myPort=%d", (long long)srcEndpoint->id, srcEndpoint->address.ToString().c_str(), (unsigned int)srcEndpoint->port, queryID, myPort);
			...
			if(waitingForRelayPeerInfo && in.Remaining()>=16){
				...
				for(vector<std::tr1::shared_ptr<Endpoint>>::iterator itrtr=endpoints.begin(); itrtr!=endpoints.end(); ++itrtr){
					std::tr1::shared_ptr<Endpoint>  ep=*itrtr;
				...
				endpoints.push_back(std::tr1::shared_ptr<Endpoint>(new Endpoint((int64_t)(FOURCC('P','2','P','4')) << 32, (uint16_t) peerPort, _peerAddr, emptyV6, Endpoint::TYPE_UDP_P2P_INET, peerTag)));
	...
	if(in.Remaining()<40){
		LOGW("VOIP recv.drop.tooSmall endpoint=%lld len=%u remaining=%u state=%d", (long long)srcEndpoint->id, (unsigned int)len, (unsigned int)in.Remaining(), (int)state);
		return;
	}
	...
	if(!useMTProto2){
		...
		if(memcmp(fingerprint, keyFingerprint, 8)!=0){
			LOGW("VOIP recv.drop.wrongFingerprint.v1 endpoint=%lld state=%d got=%02X%02X%02X%02X expected=%02X%02X%02X%02X", (long long)srcEndpoint->id, (int)state, fingerprint[0], fingerprint[1], fingerprint[2], fingerprint[3], keyFingerprint[0], keyFingerprint[1], keyFingerprint[2], keyFingerprint[3]);
			return;
		}
		...
		if(memcmp(msgHash, sha+(SHA1_LENGTH-16), 16)!=0){
			LOGW("VOIP recv.drop.wrongHash.v1 endpoint=%lld state=%d retryWith2=%d remaining=%u", (long long)srcEndpoint->id, (int)state, (int)(state==STATE_WAIT_INIT || state==STATE_WAIT_INIT_ACK), (unsigned int)in.Remaining());
			if(state==STATE_WAIT_INIT || state==STATE_WAIT_INIT_ACK)
				retryWith2=true;
			else
				return;
	...
	if(useMTProto2 || retryWith2){
		...
		if(!shortFormat){
			in.ReadBytes(fingerprint, 8);
			if(memcmp(fingerprint, keyFingerprint, 8)!=0){
				LOGW("VOIP recv.drop.wrongFingerprint.v2 endpoint=%lld state=%d got=%02X%02X%02X%02X expected=%02X%02X%02X%02X", (long long)srcEndpoint->id, (int)state, fingerprint[0], fingerprint[1], fingerprint[2], fingerprint[3], keyFingerprint[0], keyFingerprint[1], keyFingerprint[2], keyFingerprint[3]);
				return;
			}
		}
		...
		if(memcmp(msgKey, msgKeyLarge+8, 16)!=0){
			LOGW("VOIP recv.drop.wrongHash.v2 endpoint=%lld state=%d decryptedLen=%u", (long long)srcEndpoint->id, (int)state, (unsigned int)decryptedLen);
			return;
		}
...
*/
	...
	if(type==PKT_INIT){
		...
		if(buf){
			...
			for(vector<std::tr1::shared_ptr<Stream>>::iterator s=outgoingStreams.begin();s!=outgoingStreams.end();++s){
				out.WriteByte((*s)->id);
	...
	if(type==PKT_INIT_ACK){
		...
		if(!receivedInitAck){
			...
			int i;
			std::tr1::shared_ptr<Stream> incomingAudioStream;
			for(i=0; i<streamCount; i++){
				...
				if(stm->type==STREAM_TYPE_AUDIO){
					stm->jitterBuffer=std::tr1::shared_ptr<JitterBuffer>(new JitterBuffer(NULL, stm->frameDuration));
					...
					if(!stm->packetReassembler){
						stm->packetReassembler=std::tr1::shared_ptr<PacketReassembler>(new PacketReassembler());
					}
				}else{
	...
	if(type==PKT_STREAM_STATE){
		...
		for(vector<std::tr1::shared_ptr<Stream>>::iterator s=incomingStreams.begin();s!=incomingStreams.end();++s){
			if((*s)->id==id){
				(*s)->enabled=enabled==1;
	...
	if(type==PKT_LAN_ENDPOINT){
		...
		if(!found){
			...
			unsigned char peerTag[16];
			endpoints.push_back(std::tr1::shared_ptr<Endpoint>(new Endpoint((int64_t)(FOURCC('L','A','N','4')) << 32, peerPort, v4addr, v6addr, Endpoint::TYPE_UDP_P2P_LAN, peerTag)));
		}
	}
	...
	if(type==PKT_STREAM_EC){
		...
		for(std::tr1::shared_ptr<Stream>& stm:incomingStreams){
			if(stm->id==streamID){
...
void VoIPController::ProcessExtraData(Buffer &data){
	...
	if(type==EXTRA_TYPE_STREAM_FLAGS){
		...
		for(std::tr1::shared_ptr<Stream>& s:incomingStreams){
			if(s->id==id){
		...
		std::tr1::shared_ptr<Endpoint> ep=std::tr1::shared_ptr<Endpoint>(new Endpoint());
...
void VoIPController::RunTickThread(void* arg){
	...
	while(runReceiver){
		...
		if(useTCP && !didAddTcpRelays){
			vector<std::tr1::shared_ptr<Endpoint>> relays;
			for(std::tr1::shared_ptr<Endpoint>& e:endpoints){
				if(e->type!=Endpoint::TYPE_UDP_RELAY)
					continue;
				std::tr1::shared_ptr<Endpoint> tcpRelay=std::tr1::shared_ptr<Endpoint>(new Endpoint(*e));
			...
			if(tickCount%10==0 && encoder){
				...
				if(avgSendLossCount>0.125 && networkType!=NET_TYPE_GPRS && networkType!=NET_TYPE_EDGE){
					...
					if(!shittyInternetMode){
						...
						for(std::tr1::shared_ptr<Stream>& s:outgoingStreams){
		...
		if(statsDump && incomingStreams.size()==1){
			std::tr1::shared_ptr<JitterBuffer>& jitterBuffer=incomingStreams[0]->jitterBuffer;
...
void VoIPController::SendPacket(unsigned char *data, size_t len, std::tr1::shared_ptr<Endpoint> ep, PendingOutgoingPacket& srcPacket){
	if(stopping)
		return;
	...
	if(srcPacket.type==PKT_INIT || state==STATE_WAIT_INIT_ACK){
		...
		LOGI("VOIP send.packet type=%u seq=%u endpoint=%lld relayType=%d addr=%s port=%u payload=%u encrypted=%u mtproto2=%d prefixMatch=%d prefix=%02X%02X%02X%02X expected=%02X%02X%02X%02X", (unsigned int)srcPacket.type, (unsigned int)srcPacket.seq, (long long)ep->id, (int)ep->type, ep->GetAddress().ToString().c_str(), (unsigned int)ep->port, (unsigned int)len, (unsigned int)pkt.length, (int)useMTProto2, prefixMatches ? 1 : 0, pkt.data[0], pkt.data[1], pkt.data[2], pkt.data[3], expectedPrefix[0], expectedPrefix[1], expectedPrefix[2], expectedPrefix[3]);
	}
	ActuallySendPacket(pkt, ep);
}

void VoIPController::ActuallySendPacket(NetworkPacket &pkt, std::tr1::shared_ptr<Endpoint> ep){
	...
	if(itfName!=activeNetItfName){
		...
		if(currentEndpoint && currentEndpoint->type!=Endpoint::TYPE_UDP_RELAY){
			if(preferredRelay->type==Endpoint::TYPE_UDP_RELAY)
				currentEndpoint=preferredRelay;
			MutexGuard m(endpointsMutex);
			for(vector<std::tr1::shared_ptr<Endpoint>>::iterator itr=endpoints.begin();itr!=endpoints.end();){
				std::tr1::shared_ptr<Endpoint> endpoint=*itr;
...
void VoIPController::AddIPv6Relays(){
	LOGI("VOIP ipv6Relays.disabled");
	return;
	if(!myIPv6.IsEmpty() && !didAddIPv6Relays){
		std::tr1::unordered_map<string, vector<std::tr1::shared_ptr<Endpoint>>> endpointsByAddress;
		MutexGuard m(endpointsMutex);
		for(std::tr1::shared_ptr<Endpoint>& e:endpoints){
		...
		for(std::tr1::unordered_map<string, vector<std::tr1::shared_ptr<Endpoint>>>::iterator addr=endpointsByAddress.begin();addr!=endpointsByAddress.end();++addr){
			std::tr1::shared_ptr<Endpoint> best;
			int bestDiff=256;
			for(std::tr1::shared_ptr<Endpoint>& e:addr->second){
				...
				if(best){
					...
					std::tr1::shared_ptr<Endpoint> v6only=std::tr1::shared_ptr<Endpoint>(new Endpoint(*best));
...
string VoIPController::GetDebugString(){
	...
	char buffer[2048];
	for(std::tr1::shared_ptr<Endpoint>& endpoint:endpoints){
		const char* type;
	...
	double avgLate[3];
	std::tr1::shared_ptr<JitterBuffer> jitterBuffer=incomingStreams.size()==1 ? incomingStreams[0]->jitterBuffer : std::tr1::shared_ptr<JitterBuffer>();
	if(jitterBuffer)
...
std::tr1::shared_ptr<Endpoint> VoIPController::GetEndpointByType(int type){
	...
	return std::tr1::shared_ptr<Endpoint>();
}

std::tr1::shared_ptr<Endpoint> VoIPController::GetEndpointByID(int64_t id){
	for(std::tr1::shared_ptr<Endpoint>& e:endpoints){
		if(e->id==id)
			return e;
	}
...
void VoIPController::LogDebugInfo(){
	...
	for(vector<std::tr1::shared_ptr<Endpoint>>::iterator itr=endpoints.begin();itr!=endpoints.end();++itr){
		std::tr1::shared_ptr<Endpoint> e=*itr;
		char buffer[1024];
...
void VoIPController::SendUdpPing(std::tr1::shared_ptr<Endpoint> endpoint){
...
void VoIPController::SendRelayPings(){
	...
	if((state==STATE_ESTABLISHED || state==STATE_RECONNECTING) && endpoints.size()>1){
		std::tr1::shared_ptr<Endpoint> minPingRelay=preferredRelay;
		...
		if(currentEndpoint->type==Endpoint::TYPE_UDP_RELAY){
			std::tr1::shared_ptr<Endpoint> p2p=GetEndpointByType(Endpoint::TYPE_UDP_P2P_INET);
			if(p2p){
				std::tr1::shared_ptr<Endpoint> lan=GetEndpointByType(Endpoint::TYPE_UDP_P2P_LAN);
...
void VoIPController::StartAudio(){
	std::tr1::shared_ptr<Stream>& outgoingAudioStream=outgoingStreams[0];
...
void VoIPController::OnAudioOutputReady(){
	std::tr1::shared_ptr<Stream>& stm=incomingStreams[0];
	outputAGC=new AutomaticGainControl();
	outputAGC->SetPassThrough(!outputAGCEnabled);
	stm->decoder=std::tr1::shared_ptr<OpusDecoder>(new OpusDecoder(audioOutput, true, peerVersion>=6));
