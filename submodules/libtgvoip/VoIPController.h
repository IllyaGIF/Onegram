...
namespace tgvoip{
	...
	protected:
		...
		virtual void ProcessIncomingPacket(NetworkPacket& packet, std::tr1::shared_ptr<Endpoint> srcEndpoint);
		...
		virtual void SendPacket(unsigned char* data, size_t len, std::tr1::shared_ptr<Endpoint> ep, PendingOutgoingPacket& srcPacket);
		virtual void SendInit();
		virtual void SendUdpPing(std::tr1::shared_ptr<Endpoint> endpoint);
	...
	private:
		struct Stream{
			...
			std::tr1::shared_ptr<JitterBuffer> jitterBuffer;
			std::tr1::shared_ptr<OpusDecoder> decoder;
			std::tr1::shared_ptr<PacketReassembler> packetReassembler;
			std::tr1::shared_ptr<CallbackWrapper> callbackWrapper;
		...
		std::tr1::shared_ptr<Endpoint> GetEndpointByType(int type);
		std::tr1::shared_ptr<Endpoint> GetEndpointByID(int64_t id);
		...
		void LogDebugInfo();
		void ActuallySendPacket(NetworkPacket& pkt, std::tr1::shared_ptr<Endpoint> ep);
		void StartAudio();
		void ProcessAcknowledgedOutgoingExtra(UnacknowledgedExtraData& extra);
		void AddIPv6Relays();
		int state;
		std::vector<std::tr1::shared_ptr<Endpoint>> endpoints;
		std::tr1::shared_ptr<Endpoint> currentEndpoint;
		std::tr1::shared_ptr<Endpoint> preferredRelay;
		std::tr1::shared_ptr<Endpoint> peerPreferredRelay;
		bool runReceiver;
		uint32_t seq;
		...
		bool micMuted;
		uint32_t maxBitrate;
		std::vector<std::tr1::shared_ptr<Stream>> outgoingStreams;
		std::vector<std::tr1::shared_ptr<Stream>> incomingStreams;
		...
		std::tr1::unordered_map<uint8_t, uint64_t> lastReceivedExtrasByType;
		bool useIPv6;
	...
	private:
		...
		std::vector<std::tr1::shared_ptr<Stream>> DeserializeStreams(BufferInputStream& in);
		...
		struct GroupCallParticipant{
			...
			std::vector<std::tr1::shared_ptr<Stream>> streams;
		...
		int32_t userSelfID;
		std::tr1::shared_ptr<Endpoint> groupReflector;
