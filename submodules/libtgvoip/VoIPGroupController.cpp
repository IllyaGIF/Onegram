...
void VoIPGroupController::SetGroupCallInfo(unsigned char *encryptionKey, unsigned char *reflectorGroupTag, unsigned char *reflectorSelfTag, unsigned char *reflectorSelfSecret, unsigned char* reflectorSelfTagHash, int32_t selfUserID, IPv4Address reflectorAddress, IPv6Address reflectorAddressV6, uint16_t reflectorPort){
	std::tr1::shared_ptr<Endpoint> e=std::tr1::shared_ptr<Endpoint>(new Endpoint());
...
void VoIPGroupController::AddGroupCallParticipant(int32_t userID, unsigned char *memberTagHash, unsigned char* serializedStreams, size_t streamsLength){
	...
	vector<std::tr1::shared_ptr<Stream>> streams=DeserializeStreams(ss);

	unsigned char audioStreamID=0;

	for(vector<std::tr1::shared_ptr<Stream>>::iterator _s=streams.begin();_s!=streams.end();++_s){
		std::tr1::shared_ptr<Stream>& s=*_s;
		s->userID=userID;
		if(s->type==STREAM_TYPE_AUDIO && s->codec==CODEC_OPUS && !audioStreamID){
			audioStreamID=s->id;
			s->jitterBuffer=std::tr1::shared_ptr<JitterBuffer>(new JitterBuffer(NULL, s->frameDuration));
			...
			s->callbackWrapper=std::tr1::shared_ptr<CallbackWrapper>(new CallbackWrapper());
			s->decoder=std::tr1::shared_ptr<OpusDecoder>(new OpusDecoder(s->callbackWrapper, false, false));
...
void VoIPGroupController::RemoveGroupCallParticipant(int32_t userID){
	MutexGuard m(participantsMutex);
	vector<std::tr1::shared_ptr<Stream>>::iterator stm=incomingStreams.begin();
...
vector<std::tr1::shared_ptr<VoIPController::Stream>> VoIPGroupController::DeserializeStreams(BufferInputStream& in){
	vector<std::tr1::shared_ptr<Stream>> res;
	try{
		unsigned char count=in.ReadByte();
		for(unsigned char i=0;i<count;i++){
			uint16_t len=(uint16_t) in.ReadInt16();
			BufferInputStream inner=in.GetPartBuffer(len, true);
			std::tr1::shared_ptr<Stream> s=std::tr1::shared_ptr<Stream>(new Stream());
			s->id=inner.ReadByte();
...
void VoIPGroupController::SetParticipantStreams(int32_t userID, unsigned char *serializedStreams, size_t length){
	...
	for(vector<GroupCallParticipant>::iterator p=participants.begin();p!=participants.end();++p){
		if(p->userID==userID){
			...
			vector<std::tr1::shared_ptr<Stream>> streams=DeserializeStreams(in);
			for(vector<std::tr1::shared_ptr<Stream>>::iterator ns=streams.begin();ns!=streams.end();++ns){
				bool found=false;
				for(vector<std::tr1::shared_ptr<Stream>>::iterator s=p->streams.begin();s!=p->streams.end();++s){
					if((*s)->id==(*ns)->id){
...
void VoIPGroupController::ProcessIncomingPacket(NetworkPacket &packet, std::tr1::shared_ptr<Endpoint> srcEndpoint){
	...
	if(type==PKT_STREAM_DATA || type==PKT_STREAM_DATA_X2 || type==PKT_STREAM_DATA_X3){
		...
		for(i=0;i<count;i++){
			...
			for(vector<std::tr1::shared_ptr<Stream>>::iterator stm=sender->streams.begin();stm!=sender->streams.end();++stm){
				if((*stm)->id==streamID){
					if((*stm)->jitterBuffer){
...
void VoIPGroupController::SendUdpPing(std::tr1::shared_ptr<Endpoint> endpoint){
...
void VoIPGroupController::SendPacket(unsigned char *data, size_t len, std::tr1::shared_ptr<Endpoint> ep, PendingOutgoingPacket& srcPacket){
	if(stopping)
		return;
...
std::string VoIPGroupController::GetDebugString(){
	...
	for(vector<GroupCallParticipant>::iterator p=participants.begin();p!=participants.end();++p){
		...
		r+=buffer;
		for(vector<std::tr1::shared_ptr<Stream>>::iterator stm=p->streams.begin();stm!=p->streams.end();++stm){
