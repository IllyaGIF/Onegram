...
void AudioUnitIO::Configure(uint32_t sampleRate, uint32_t bitsPerSample, uint32_t channels){
	...
	audioFormat.mFormatFlags = kAudioFormatFlagIsFloat | kAudioFormatFlagIsPacked | kAudioFormatFlagsNativeEndian;
	audioFormat.mBitsPerChannel = 32;
	audioFormat.mBytesPerPacket = 4 * channels;
	audioFormat.mBytesPerFrame = 4 * channels;
#endif
	audioFormat.mFramesPerPacket = 1;
	audioFormat.mChannelsPerFrame = channels;

	OSStatus outStatus = AudioUnitSetProperty(unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, kOutputBus, &audioFormat, sizeof(audioFormat));
	OSStatus inStatus = AudioUnitSetProperty(unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, kInputBus, &audioFormat, sizeof(audioFormat));
	if(outStatus != noErr || inStatus != noErr){
		syslog(LOG_ERR, "LAT audioUnit configure requested=%u outStatus=%d inStatus=%d", (unsigned int)sampleRate, (int)outStatus, (int)inStatus);
		failed=true;
		return;
	}
	...
	AudioStreamBasicDescription actualIn;
	memset(&actualOut, 0, sizeof(actualOut));
	memset(&actualIn, 0, sizeof(actualIn));
	UInt32 outSize=sizeof(actualOut);
	UInt32 inSize=sizeof(actualIn);
	OSStatus getOut=AudioUnitGetProperty(unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Input, kOutputBus, &actualOut, &outSize);
	OSStatus getIn=AudioUnitGetProperty(unit, kAudioUnitProperty_StreamFormat, kAudioUnitScope_Output, kInputBus, &actualIn, &inSize);
	syslog(LOG_NOTICE, "LAT audioUnit format requested=%u outRate=%.0f inRate=%.0f outGet=%d inGet=%d",
		(unsigned int)sampleRate,
		getOut==noErr ? actualOut.mSampleRate : 0.0,
		getIn==noErr ? actualIn.mSampleRate : 0.0,
		(int)getOut,
		(int)getIn);
}
