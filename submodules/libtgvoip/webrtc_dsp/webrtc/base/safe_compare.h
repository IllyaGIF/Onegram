...
#include "webrtc/base/legacy_type_traits.h"
...
template <size_t N>
struct LargerIntImpl : legacy_traits::false_type {};
template <>
struct LargerIntImpl<sizeof(int8_t)> : legacy_traits::true_type {
  using type = int16_t;
};
template <>
struct LargerIntImpl<sizeof(int16_t)> : legacy_traits::true_type {
  using type = int32_t;
};
template <>
struct LargerIntImpl<sizeof(int32_t)> : legacy_traits::true_type {
  using type = int64_t;
};
...
template <typename T>
inline typename legacy_traits::make_unsigned<T>::type MakeUnsigned(T a) {
  return static_cast<typename legacy_traits::make_unsigned<T>::type>(a);
}
...
template <typename Op,
          typename T1,
          typename T2,
          typename legacy_traits::enable_if<legacy_traits::is_signed<T1>::value ==
                                  legacy_traits::is_signed<T2>::value>::type* = nullptr>
...
template <typename Op,
          ...
          typename T2,
          typename legacy_traits::enable_if<legacy_traits::is_signed<T1>::value &&
                                  legacy_traits::is_unsigned<T2>::value &&
...
template <typename Op,
          typename T1,
          typename T2,
          typename legacy_traits::enable_if<legacy_traits::is_unsigned<T1>::value &&
                                  legacy_traits::is_signed<T2>::value &&
...
#define RTC_SAFECMP_MAKE_FUN(name)                                             \
  template <                                                                   \
      ...
      typename legacy_traits::enable_if<                                                 \
          legacy_traits::is_integral<typename legacy_traits::remove_reference<T1>::type>::value && \
          legacy_traits::is_integral<typename legacy_traits::remove_reference<T2>::type>::value>:: \
          type* = nullptr>                                                     \
  ...
  template <typename T1, typename T2,                                          \
            typename legacy_traits::enable_if<                                           \
                !legacy_traits::is_integral<                                             \
                    typename legacy_traits::remove_reference<T1>::type>::value ||        \
                !legacy_traits::is_integral<typename legacy_traits::remove_reference<T2>::type>::  \
                    value>::type* = nullptr>                                   \
  inline bool name(T1&& a, T2&& b) {                                           \
