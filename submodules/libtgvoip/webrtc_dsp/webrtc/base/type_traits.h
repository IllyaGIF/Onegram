...
class HasDataAndSize {
 private:
  ...
  static auto Test(int)
      -> decltype(legacy_traits::Accept<T*>(legacy_traits::Declval<C>().data()),
                  legacy_traits::Accept<std::size_t>(legacy_traits::Declval<C>().size()),
                  legacy_traits::true_type());

  template <typename>
  static legacy_traits::false_type Test(...);

 public:
  static const bool value = decltype(Test<DS>(0))::value;
};
