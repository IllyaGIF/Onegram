...
static bool STPThemeGetHSBA(UIColor *color, CGFloat *hue, CGFloat *saturation, CGFloat *brightness, CGFloat *alpha)
{
    ...
    CGFloat h = 0.0f;
    CGFloat sat = maximum <= 0.000001f ? 0.0f : delta / maximum;
    if (delta > 0.000001f)
    {
        if (maximum == red)
            h = (green - blue) / delta;
...
- (UIColor *)tertiaryBackgroundColor {
	...
	CGFloat brightness;
	CGFloat alpha;
	STPThemeGetHSBA(self.primaryBackgroundColor, &hue, &saturation, &brightness, &alpha);
