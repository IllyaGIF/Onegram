...
+ (AVCaptureDeviceFormat *)selectCaptureDeviceFormatForDevice:(AVCaptureDevice *)selectedCamera {
    ...
    for (int i = (int)[sortedFormats count] - 1; i >= 0; i--) {
        if ([[[sortedFormats[i] description] lowercaseString] rangeOfString:@"x420"].location != NSNotFound) {
...
+ (VideoCapturerInterfaceImplSourceDescription *)selectCapturerDescriptionWithDeviceId:(NSString *)deviceId {
    ...
    if (selectedCamera == nil) {
        return [[VideoCapturerInterfaceImplSourceDescription alloc] initWithIsFrontCamera:![deviceId hasPrefix:@"back"] keepLandscape:[deviceId rangeOfString:@"landscape"].location != NSNotFound deviceId: deviceId device: nil format: nil];
    }

    AVCaptureDeviceFormat *bestFormat = [VideoCapturerInterfaceImplReference selectCaptureDeviceFormatForDevice:selectedCamera];

    return [[VideoCapturerInterfaceImplSourceDescription alloc] initWithIsFrontCamera:![deviceId hasPrefix:@"back"] keepLandscape:[deviceId rangeOfString:@"landscape"].location != NSNotFound deviceId:deviceId device:selectedCamera format:bestFormat];
}
